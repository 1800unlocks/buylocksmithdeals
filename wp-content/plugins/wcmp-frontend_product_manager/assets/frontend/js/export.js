jQuery(document).ready(function($) {	
	$('.woocommerce-exporter-progress').hide();
	$('#woocommerce-exporter-columns').select2();
	$('#woocommerce-exporter-types').select2();
	$('#woocommerce-exporter-category').select2();
	//$('.wcmp_booking_tbl,.wcmp_auction_tbl').DataTable();
	...
	;(function ( $, window ) {
		...
		productExportForm.prototype.processStep = function( step, data, columns, filename ) {
			...
			$.ajax( {
				type: 'POST',
				url: wc_product_export_params.ajax_url,
				data: {
					form             : data,
