 ...
 *
 * @class       WC_Email_Vendor_New_Booking
 * @extends     WC_Email
 */
class WC_Email_Vendor_New_Booking extends WC_Email {
	/**
	...
	public function __construct() {
		...
		$this->template_html  = 'booking/emails/vendor-new-booking.php';
		$this->template_plain = 'booking/emails/plain/vendor-new-booking.php';
		...
		add_action( 'woocommerce_vendor_new_booking_notification', array( $this, 'trigger' ) );
	...
	public function queue_notification( $booking_id ) {
		wp_schedule_single_event( time(), 'woocommerce_vendor_new_booking_notification', array( 'booking_id' => $booking_id ) );
	}
	...
	public function trigger( $booking_id ) {
		if ( $booking_id ) {
			...
			$booking_order = $this->object->get_order_id();
            $suborder = get_wcmp_suborders($booking_order);
			...
			if ( $this->object->get_order() ) {
				...
				$this->replace[] = $suborder[0]->get_id();
			} else {
				$this->find[]    = '{order_date}';
