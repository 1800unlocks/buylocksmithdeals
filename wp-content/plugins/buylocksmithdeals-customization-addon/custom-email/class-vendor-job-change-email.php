 ...
 *
 */
class Blsd_Vendor_Job_Change_Email extends WC_Email {
    
    function __construct() {
        
        // Add email ID, title, description, heading, subject
        $this->id                   = 'blsd_vendor_job_change_email';
        $this->title                = __( 'Blsd Vendor Job Change Email', 'blsd-custom-email' );
        $this->description          = __( 'This email is received when an order status is changed to Pending.', 'blsd-custom-email' );
        
        $this->heading              = __( 'Blsd Vendor Job Change Email', 'blsd-custom-email' );
        $this->subject              = __( 'Order Job Updated', 'blsd-custom-email' );
        
        // email template path
        $this->template_html    = 'emails/custom-blsd-Job-change-email.php';
        $this->template_plain   = 'emails/plain/custom-blsd-Job-change-email.php';
        
        // Triggers for this email
        add_action( 'vendor_job_change_email_notification', array( $this, 'trigger' ) );
    ...
    function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                'title' 		=> __( 'Enable/Disable', 'blsd-custom-email' ),
                'type' 			=> 'checkbox',
                'label' 		=> __( 'Enable this email notification', 'blsd-custom-email' ),
                'default' 		=> 'yes'
            ),
            'recipient' => array(
                'title'         => __( 'Recipient', 'blsd-custom-email' ),
                'type'          => 'text',
                'description'   => sprintf( __( 'Enter recipients (comma separated) for this email. Defaults to %s', 'blsd-custom-email' ), get_option( 'admin_email' ) ),
                'default'       => get_option( 'admin_email' )
            ),
            'subject' => array(
                'title' 		=> __( 'Subject', 'blsd-custom-email' ),
                'type' 			=> 'text',
                'description' 	=> sprintf( __( 'This controls the email subject line. Leave blank to use the default subject: <code>%s</code>.', 'blsd-custom-email' ), $this->subject ),
                'placeholder' 	=> '',
                'default' 		=> ''
            ),
            'heading' => array(
                'title' 		=> __( 'Email Heading', 'blsd-custom-email' ),
                'type' 			=> 'text',
                'description' 	=> sprintf( __( 'This controls the main heading contained within the email notification. Leave blank to use the default heading: <code>%s</code>.', 'blsd-custom-email' ), $this->heading ),
                'placeholder' 	=> '',
                'default' 		=> ''
            ),
            'email_type' => array(
                'title' 		=> __( 'Email type', 'blsd-custom-email' ),
                'type' 			=> 'select',
                'description' 	=> __( 'Choose which format of email to send.', 'blsd-custom-email' ),
                'default' 		=> 'html',
                'class'			=> 'email_type',
                'options'		=> array(
                    'plain'		 	=> __( 'Plain text', 'blsd-custom-email' ),
                    'html' 			=> __( 'HTML', 'blsd-custom-email' ),
                    'multipart' 	=> __( 'Multipart', 'blsd-custom-email' ),
                )
            )
        );
    }
...
return new Blsd_Vendor_Job_change_Email();
?>