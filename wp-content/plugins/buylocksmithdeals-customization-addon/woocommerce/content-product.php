...
$more_link_text = sprintf(wp_kses(__('Read more<span class="meta-nav"> &rsaquo;</span>', 'AZEXO'), array('span' => array('class' => array()))));
...
<div class="entry">
    ...
    <div class="entry-data">
        <div class="entry-header"><?php
			...
			$discount_on_deal=get_post_meta(get_the_ID(), 'discount_on_deal', true);
			if(!empty($discount_on_deal)){
				if(is_product()){
					$discount=$discount_on_deal.'% Off';
				}
				else{
					$discount=$discount_on_deal.'% Discount Today';
				}
				
            ?>
			<div class="discount_show"><span class="discount_percent"><?php  echo $discount; ?></span></div>
			<?php
			 } 
            ...
            ?><?php if (!empty($meta)) : ?>
                <div class="entry-meta price_starting"><?php   print '<span class="starting_at">Starting at</span>'.azexo_entry_meta($product_template, 'meta'); ?></div>
            <?php endif; ?><?php
...
?>
<style>
    .shop-product .vendor_description_background .wcmp_vendor_banner>img {
    width: 100%;
    border-radius: 0;
    max-height: 220px;
    object-fit: cover;
    }
    .price_starting span.starting_at {
    position: absolute;
    top: 23px;
    right: 115px;
}
...
.product_type_simple, .product_type_booking {
