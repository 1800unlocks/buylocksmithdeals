    ...
    <div id="comments">
        <h2><?php
            if (get_option('woocommerce_enable_review_rating') === 'yes' && ( $count = azexo_review_count($post) )) {
                printf(_n('%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'woocommerce'), $count, get_the_title());
            } else {
            ...
            <?php if (isset($options['show_ratings_summary']) && $options['show_ratings_summary']) : ?>
                <div class="ratings-summary">
                    <div class="average-rating">
                        ...
                        <label><?php esc_html_e('Overall rating', 'AZEXO'); ?></label>
                    </div>
                    <div class="stars">
                        <?php woocommerce_template_single_rating(); ?>
                    </div>         
            ...
            <div id="review_form">
                ...
                $comment_form = array(
                    ...
                    'fields' => array(
                        ...
                        '<input id="author" name="author" type="text" placeholder="' . (isset($options['review_form_placeholders']) && $options['review_form_placeholders'] ? esc_attr__('Name', 'woocommerce') : '') . '" value="' . esc_attr($commenter['comment_author']) . '" size="30" aria-required="true" /></p>',
                        ...
                        '<input id="email" name="email" type="text" placeholder="' . (isset($options['review_form_placeholders']) && $options['review_form_placeholders'] ? esc_attr__('Email', 'woocommerce') : '') . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="30" aria-required="true" /></p>',
                    ),
                ...
                $comment_form['comment_field'] .= '<p class="comment-form-comment">' . (isset($options['review_form_placeholders']) && $options['review_form_placeholders'] ? '' : '<label for="comment">' . esc_html__('Your review', 'woocommerce') . '</label>') . '<textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" placeholder="' . (isset($options['review_form_placeholders']) && $options['review_form_placeholders'] ? esc_attr__('Your review', 'woocommerce') : '') . '"></textarea></p>';
