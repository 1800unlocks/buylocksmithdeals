...
if(isset($_POST['store_save'])){
    update_user_meta($vendor->id,'_vendor_csd_return_address1',$_POST['vendor_address_1']);
    update_user_meta($vendor->id,'_vendor_csd_return_address2',$_POST['vendor_address_2']);
    update_user_meta($vendor->id,'_vendor_csd_return_country',$_POST['vendor_country']);
    update_user_meta($vendor->id,'_vendor_csd_return_state',$_POST['vendor_state']);
    update_user_meta($vendor->id,'_vendor_csd_return_city',$_POST['vendor_city']);
    update_user_meta($vendor->id,'_vendor_csd_return_zip',$_POST['vendor_postcode']);
    update_user_meta($vendor->id,'_vendor_customer_phone',$_POST['vendor_phone']);
    update_user_meta($vendor->id,'_vendor_customer_email',$_POST['vendor_email']);
	update_user_meta($vendor->id,'_vendor_mobile_phone',$_POST['vendor_mobile_phone']);
    $user_id = $vendor->id;
    $website = $_POST['vendor_website'];
    $user_data = wp_update_user( array( 'ID' => $user_id, 'user_url' => $website ) );         
    
        update_user_meta($vendor->id,'vendor_1800_unlocks_profile',$_POST['vendor_1800_unlocks_profile']);
}
$vendor = get_current_vendor();
...
$vendor_mobile_phone = get_user_meta($vendor->id, '_vendor_mobile_phone', true);
        ...
        <div class="panel panel-default panel-pading pannel-outer-heading">
            ...
            <div class="panel-body panel-content-padding">
                <div class="wcmp_form1">
                    ...
                    <div class="form-group">
                        <label class="control-label col-sm-3 col-md-3"><?php _e(' Store URL *', 'dc-woocommerce-multi-vendor'); ?></label>
                        <div class="col-md-6 col-sm-9">
                            <div class="input-group">
                                ...
                                <input class="small no_input form-control" onkeyup="checkexistingSlug(jQuery(this))" id="basic-url" aria-describedby="basic-addon3" type="text" name="vendor_page_slug" value="<?php echo isset($vendor_page_slug['value']) ? $vendor_page_slug['value'] : ''; ?>" placeholder="<?php _e('Enter your Store Name here', 'dc-woocommerce-multi-vendor'); ?>">
					...
					<div class="form-group">
                        <label class="control-label col-sm-3 col-md-3"><?php _e('Mobile Phone', 'dc-woocommerce-multi-vendor'); ?></label>
                        <div class="col-md-6 col-sm-9">
                            <input class="no_input form-control" type="text" name="vendor_mobile_phone" placeholder="" value="<?php echo isset($vendor_mobile_phone) ? $vendor_mobile_phone : ''; ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-sm-3 col-md-3"><?php _e('Email *', 'dc-woocommerce-multi-vendor'); ?></label>
                        <div class="col-md-6 col-sm-9">                            
                            <input class="no_input vendor_email form-control" name="vendor_email" type="text" placeholder="" readonly  value="<?php echo isset($vendor->user_data->user_email) ? $vendor->user_data->user_email : ''; ?>">
                        </div>
                    </div>
                    <div class="form-group">
        ...
        <div class="panel panel-default pannel-outer-heading">
            ...
            <div class="panel-body panel-content-padding form-horizontal">
                <div class="wcmp_media_block">
                    ...
                    <div class="form-group">
                        <label class="control-label col-sm-3 col-md-3 facebook"><?php _e('Website', 'dc-woocommerce-multi-vendor'); ?></label>
                        <div class="col-md-6 col-sm-9">
                            <input class="form-control" type="url"   name="vendor_website" value="<?php echo isset($vendor->user_data->user_url) ? $vendor->user_data->user_url : ''; ?>">
                        </div>  
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label class="control-label col-sm-3 col-md-3 google-plus"><?php _e('Google Maps', 'dc-woocommerce-multi-vendor'); ?></label>
                        <div class="col-md-6 col-sm-9">
                    ...
                    <div class="form-group">
                     ...
                     <div class="form-group">
                        <label class="control-label col-sm-3 col-md-3 instagram"><?php _e('1-800-Unlocks Profile', 'dc-woocommerce-multi-vendor'); ?></label>
                        <div class="col-md-6 col-sm-9">
                            <?php 
                            
 $vendor_1800_unlocks_profile = get_user_meta(get_current_user_id(),'vendor_1800_unlocks_profile',1);

                            ?>
                            <input class="form-control" type="url"   name="vendor_1800_unlocks_profile" value="<?php echo $vendor_1800_unlocks_profile ? $vendor_1800_unlocks_profile : ''; ?>">
                        </div>  
                    </div>
...
<script>
    var timeVar = '';
    var error = 0;
function checkexistingSlug(that){
       error = 0;
         jQuery('.input_error').removeClass('error');
         jQuery('.input_error').html('');
    var slug = jQuery(that).val();
    clearTimeout(timeVar);
  timeVar =  setTimeout(function(){ get_status_slug(slug) }, 1300);
    var format = /[ !@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/;
    if(format.test(slug)){
       error = 1;
    }
    if(error){
         clearTimeout(timeVar);
         jQuery('.input_error').addClass('error');
         jQuery('.input_error').html('Space or special characters not allowed.');
         
    }
}


function get_status_slug(slug){
     jQuery('.image_loader').show();   
         jQuery("button[name='store_save']").attr("disabled", true);
     var urlll = '<?php echo add_query_arg( 'action', 'blsd_slug_duplicate_status', $WCMp->ajax_url() ); ?>';
      jQuery.ajax({
      type: 'POST',
      url: urlll,
      data: 'slug='+slug,
    //  dataType: "text",
      success: function(resultData) {
          ...
          if(!resultData.status){
               jQuery('.input_error').addClass('error');
         jQuery('.input_error').html('Slug Already exists.');
        //  jQuery("button[name='store_save']").attr("disabled", true);
        }else{
         jQuery("button[name='store_save']").removeAttr('disabled');   
        }
