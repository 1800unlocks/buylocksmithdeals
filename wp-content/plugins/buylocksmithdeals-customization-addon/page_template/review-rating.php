...
$vendor_business_name = get_user_meta($vendor_id, '_vendor_page_title', true);
...
<div id="main" class="site-main">
    <div class="container active-sidebar right">
        ...
        <div class="confirmation-box">
            ...
            <?php
            if(empty($suborder)){
                echo '<div class="error-msg"><span>You are not Authorized to give rating on this order.</span></div>';
            }
            else{
            if($given_rating){
                 echo '<div class="error-msg"><span>You have already provided a review for this purchase. You cannot leave more than one review for the same purchase.</span></div>';
            }
            else{ ?>
                        ...
                        <div id="review_form_wrapper">
                            <div id="review_form">
                                ...
                                <h3 id="reply-title" class="comment-reply-title"><?php
                                        ...
                                        echo sprintf(__('Add a review to "%s"', 'dc-woocommerce-multi-vendor'), $vendor_business_name);
                                ...
                                <form action="" method="post" id="commentform" class="comment-form" novalidate="">
                                    ...
                                    <p class="comment-form-rating"><label for="rating"><?php echo __('Your Rating', 'dc-woocommerce-multi-vendor'); ?></label>					
                                     <p class="stars"><span><a class="star-1" href="#">1</a><a class="star-2" href="#">2</a><a class="star-3" href="#">3</a><a class="star-4" href="#">4</a><a class="star-5" href="#">5</a></span></p>   
                                        ...
                                        <textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" class=""></textarea>
                                    </p>
                                    <p class="form-submit">
...
#reviews #review_form #submit {
    ...
    box-shadow: 0 1px 3px 0 rgb(120, 200, 79), 0 1px 2px 0 rgb(120, 200, 79);
    outline: 0;
}
div#respond {
    background: #f2f5f7;
...
.confirmation-box {
 ...
 .confirmation-box p.comment-form-comment label{
    display: block;
    text-align: left;
    max-width: 338px;
    margin: 0 auto; 
...
.confirmation-box p.comment-form-comment textarea {
    height: 40px;
 ...
 .confirmation-box p.comment-form-rating label {
    font-size:22px;
}
  .confirmation-box p.stars{
    font-size: 20px;
}
 #reviews #review_form #submit{
    ...
    jQuery('body')
            ...
            .on('click', '#wcmp_vendor_reviews #respond #submit', function () {
                ...
                if ($rating.size() > 0 && !rating) {
                    window.alert('Please rate the vendor');
                    return false;
                }
				var comment = jQuery('#wcmp_vendor_reviews #respond #comment').val();
                if (comment == '' || comment.length < 10) {
                    window.alert('Please review your vendor and minimum 10 Character required');
                    return false;
                }
