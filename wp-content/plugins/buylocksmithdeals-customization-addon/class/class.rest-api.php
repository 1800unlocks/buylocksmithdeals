...
class BLSDRestApi {
    ...
    public static function register_rest_route() {
		...
		register_rest_route(self::$restApiBase, '/get_products_corousel', array(
            'methods' => 'POST',
            'callback' => array(__CLASS__, 'get_products_html_corousel'),
        ));
    }
...
//            print_r($products); exit;
    ...
    public static function get_products_html_corousel(WP_REST_Request $request){
		global $wpdb;
        $request_params = self::parse_request_params($request);
					...
					$html .='<div class="owl-slider">
						...
						foreach ($products as $product) {	
							...
							if (count($handle->get_attributes()) == 0) { //Condition for skip variation products
								...
								$discount_on_deal=get_post_meta($product->ID, 'discount_on_deal', true);	
								...
								$html .='<div class="item">
									...
									<div class="content">';
									if(!empty($discount_on_deal)){
										$html .='<div class="discout-today-label"><span>'.$discount_on_deal.'% Discount Today</span></div>';
									}
										
									$html .='	<h2>' . $product->post_title . '</h2>
										<span class="price">Starting at ' . $currency . ' <b>' . $price . '</b></span>
										<div class="buy-now-btn"><a href="' . get_permalink($product->ID) . '" target="_blank">Buy Now</a></div>			
									</div>
