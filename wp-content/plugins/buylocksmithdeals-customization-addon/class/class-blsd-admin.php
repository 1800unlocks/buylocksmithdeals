...
final class BuyLockSmithDealsCustomizationAdmin {
	...
	remove_action( 'woocommerce_product_data_panels', array($pinstance,'booking_panels'), 10 );
       ...
       add_action('admin_menu', array($this, 'theme_options_panel'), 1000);
		...
		add_action( 'woocommerce_setup_footer',array($this,'blsd_apply_css_admin'),10 );
    }
    
   public static function blsd_apply_css_admin(){
...
margin-left: 1px;
    ...
    public static function blsd_add_custom_product_data_fields(){
        ...
        $post_id=$post->ID;
			...
			$is_mobile_service = get_post_meta($post_id, 'is_mobile_service', true);
			...
			$need_key_to_work=  get_post_meta( $post_id, 'need_key_to_work', true );
				/***********Car Lockout********/
				$quantity_of_car_unlocks = get_post_meta( $post_id, 'quantity_of_car_unlocks', true );
				/*****************************/
				
				/************CAR Key Programming**********/
				$is_vats_programming = get_post_meta( $post_id, 'is_vats_programming', true );
				...
				$is_in_shop_pricing_cp =  get_post_meta( $post_id, 'is_in_shop_pricing_cp', true );
			...
			$have_any_working_keys_locks = get_post_meta( $post_id, 'have_any_working_keys_locks', true );
				...
				$in_shop_pricing = get_post_meta($post_id, 'in_shop_pricing', true);
				//$cost_cut_standard_house_key=  get_post_meta($post_id, 'cost_cut_standard_house_key', true);
				/****************************/
				
				/*****Home Lockout Service****/
				$quantity_of_door_locks = get_post_meta( $post_id, 'quantity_of_door_locks', true );
				/****************************/
			/*********************/
       ...
       <div id="blsd_add_custom_product_data_fields" class="panel woocommerce_options_panel">
		   ...
		   <div class="single_setting_custom ">
               <input type="checkbox" <?php if($is_mobile_service == 'yes'){ echo 'checked'; } ?> name="is_mobile_service" value="1" class="setting_ask"> Is This Deal Only Offered At The Customer’s Location?
           </div>
		   <div class="single_setting_custom ">
		   ...
		   <div class="single_setting_custom">
			<label> <b>Select Deal Types</b></label><br><br>
				<div class="deal_main_div">
				<input type="radio" name="deal_type" class="deal-type" <?php if($deal_type == 'deadbolt'){ echo 'checked'; } ?> value="deadbolt" data-id="deadbolt_installation">Deadbolt Installation
				<br>
				<input type="radio" name="deal_type" class="deal-type" <?php if($deal_type == 'lock_rekeying'){ echo 'checked'; } ?> value="lock_rekeying" data-id="property">Lock Rekeying
				<br><input type="radio" name="deal_type" class="deal-type" <?php if($deal_type == 'home_lockout'){ echo 'checked'; } ?> value="home_lockout" data-id="property">Home Lockout Service 
				<br><input type="radio" name="deal_type" class="deal-type" <?php if($deal_type == 'car_lockout'){ echo 'checked'; } ?> value="car_lockout" data-id="car">Car Lockout
				<br><input type="radio" name="deal_type" class="deal-type" <?php if($deal_type == 'car_key_programming'){ echo 'checked'; } ?> value="car_key_programming" data-id="car">Car Key Programming
				</div>
			</div>
			
		   <div id="car" class="deal-main-div single_setting_custom disabled">
			   ...
			   <div id="car_lockout" class="deal-sub-div single_setting_custom disabled">
					<label><b>Car Lockout</b></label><br>
					<div class="single_setting_custom ">
					   <input type="checkbox" <?php if($quantity_of_car_unlocks == 'yes'){ echo 'checked'; } ?> name="quantity_of_car_unlocks" value="1" class="setting_ask">Ask Quantity of cars you will unlock?
					</div>
			   </div>
			   <div id="car_key_programming" class="deal-sub-div single_setting_custom disabled">
					<label><b>Car Key Programming</b></label><br>
					<div class="single_setting_custom ">
					   <input type="checkbox" <?php if($is_vats_programming == 'yes'){ echo 'checked'; } ?> name="is_vats_programming" value="1" class="setting_ask"> Is VATS Programming?
					</div> 
				   <div class="single_setting_custom ">
				   ...
				   <div class="single_setting_custom ">
					   <input type="checkbox" <?php if($is_in_shop_pricing_cp == 'yes'){ echo 'checked'; } ?> name="is_in_shop_pricing_cp" value="1" class="setting_ask"> In-Shop Pricing?
				   </div>
				   <!--<div class="single_setting_custom ">
		   ...
		   <div id="property" class="deal-main-div single_setting_custom disabled">
				<label><b>PROPERTY</b></label><br>
				<div class="single_setting_custom ">
				...
				<div id="lock_rekeying" class="deal-sub-div single_setting_custom disabled">
				<label><b>Lock Rekeying</b></label><br>
					<div class="single_setting_custom ">
				   ...
				   <div class="single_setting_custom ">
					   <input type="checkbox" <?php if($in_shop_pricing == 'yes'){ echo 'checked'; } ?> name="in_shop_pricing" value="1" class="setting_ask">In-Shop Pricing
					</div>
					
				</div>
			   <div id="home_lockout" class="deal-sub-div single_setting_custom disabled">
					<label><b>Home Lockout Service</b></label><br>
					<div class="single_setting_custom">
					   <input type="checkbox" <?php if($quantity_of_door_locks == 'yes'){ echo 'checked'; } ?> name="quantity_of_door_locks" value="1" class="setting_ask">Ask Quantity of door locks you will unlock?
					</div>
			   </div>
		   </div>
		   
		   <div id="deadbolt_installation" class="deal-main-div single_setting_custom disabled">
				<label><b>DEADBOLT</b></label><br>
			   <div id="deadbolt" class="deal-sub-div single_setting_custom disabled" >
			   		<div class="single_setting_custom ">
					   <input type="checkbox" <?php if($where_property_located == 'yes'){ echo 'checked'; } ?> name="where_property_located" value="1" class="setting_ask"> Where is the property located?
					</div>
					<div class="single_setting_custom ">
			...
			jQuery(document).ready(function(){
				var deal_value_checked = jQuery(".deal-type:checked"). val();
				var deal_attr_checked = jQuery(".deal-type:checked"). attr('data-id');
				select_deal_type_div(deal_value_checked,deal_attr_checked);
				function select_deal_type_div(deal_value,maindeal){
					jQuery('.deal-main-div').addClass('disabled');
					jQuery('.deal-sub-div').addClass('disabled');
					jQuery('#'+maindeal).removeClass('disabled');
					jQuery('#'+deal_value).removeClass('disabled');
				}
				
				jQuery('.deal-type').click(function(){
					var deal_value=jQuery(this).val();
					var maindeal=jQuery(this).attr('data-id');
					select_deal_type_div(deal_value,maindeal);
					
				});
			});
			</script>
           <style>
		   .single_setting_custom input {
				padding: 0;
				margin: 0px 10px 5px 4px !important;
			}
			   .single_setting_custom label {
					float: none;
					margin: 0;
				}
...
//    wp_enqueue_script('custom_wp_colorpicker_js');
   ...
   $name = '';
        ...
        $is_in_shop_pricing_cp = isset( $_POST['is_in_shop_pricing_cp'] ) ? 'yes' : 'no';
        ...
        $is_vats_programming = isset( $_POST['is_vats_programming'] ) ? 'yes' : 'no';
        $quantity_of_car_unlocks = isset( $_POST['quantity_of_car_unlocks'] ) ? 'yes' : 'no';
        $quantity_of_door_locks = isset( $_POST['quantity_of_door_locks'] ) ? 'yes' : 'no';
        $house_keys_included = isset( $_POST['house_keys_included'] ) ? 'yes' : 'no';
		$in_shop_pricing = isset( $_POST['in_shop_pricing'] ) ? 'yes' : 'no';
        ...
        $is_mobile_service = isset( $_POST['is_mobile_service'] ) ? 'yes' : 'no';
        $default_my_location_price = isset( $_POST['default_my_location_price'] ) ? $_POST['default_my_location_price'] : 0;
       
        
        
        
        update_post_meta( $post_id, 'is_mobile_service', $is_mobile_service );
        ...
        update_post_meta( $post_id, 'is_in_shop_pricing_cp', $is_in_shop_pricing_cp );
        ...
        update_post_meta( $post_id, 'is_vats_programming', $is_vats_programming );
        update_post_meta( $post_id, 'quantity_of_car_unlocks', $quantity_of_car_unlocks );
        update_post_meta( $post_id, 'quantity_of_door_locks', $quantity_of_door_locks );
        update_post_meta( $post_id, 'house_keys_included', $house_keys_included );
		
        update_post_meta( $post_id, 'in_shop_pricing', $in_shop_pricing );
