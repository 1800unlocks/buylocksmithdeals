   ...
   function blsd_add_js_frontend(){
		...
		if($deal_type == 'lock_rekeying' || $deal_type == 'home_lockout' || $deal_type == 'deadbolt'){
			...
			if($deal_type == 'deadbolt'){
				...
				if($quantity_of_door_locks == 'yes'){
					...
					. '*This deal will only includes ' . $quantity_of_door_locks_unlocks . ' house unlock.'
					. '</span>'
					. '</div>';
				}
			...
			if($deal_type == 'car_lockout'){
				if($quantity_of_car_unlocks == 'yes'){
					echo '<div class="options_show"><label> Car Locks to unlock</label>';
					echo '<input type="hidden" name="quantity_of_cars_unlocks" id="quantity_of_cars_unlocks" value="'.$quantity_of_cars_unlocks.'">';
					echo '<span class="chat_support">'
					. '*This deal will only includes ' . $quantity_of_cars_unlocks . ' car unlock.'
					. '</span>'
					. '</div>';
				}
			}
				...
				if($want_more_than_one_key == 'yes'){
				  ...
				  $option = '<option value="">Choose number of keys</option>';
					...
					. $option
					. '</select><span class="chat_support">*This deal is for the cutting and programming of 1 car key. If you want more done, please choose above. The locksmith will provide a quote for additional materials.</span></div>';
       ...
       <script>
			...
			function calculate_deal_pricing_car_key_programming(){
				...
				if (service_location_price > 0 && service_location_val != '') {
					...
					var key_cost=parseFloat(no_of_keys_want_to_made*total_key_cost);
                             ...
                             var total_service_booking_amount=parseFloat(booking_price+car_key_price+key_cost+extra_cylinder_price+car_programming_fees+deadbolt_install_price+extra_deadbolt_cylinder_price+extra_house_key_price);
            ...
            jQuery('.wc-pao-addon-select option:contains("None")').text('Choose service location');
				...
				function deadbolts_extra_quantity_locks_install(type_of_installation,who_is_supplying_deadbolts,quantity_locks_to_install){
					if(type_of_installation !='' && who_is_supplying_deadbolts !='' && quantity_locks_to_install !=''){
						...
						if(quantity_locks_to_install > quantity_deadbolt){
						 ...
						 calculate_final_deadbolt_amount(0,extra_per_deadbolt_cylinders_price);   
						}
					}
				}
				...
				function calculate_final_deadbolt_amount(extra_deadbolt_cylinders,extra_per_deadbolt_cylinders_price){
                    ...
                    if (element.classList.contains("property")) {
                        ...
                        console.log(amount, total, service_location_price, booking_price, booking_price_with_tax,extra_deadbolt_cylinder_price);
				...
				jQuery('#quantity_of_extra_house_keys').change(function(){
                    ...
                    if(included_house_key < quantity_of_extra_house_keys){
                        ...
                        console.log('extra_house_key', extra_house_keys);
                        console.log('extra_cylinders_price', extra_cylinders_price);
                ...
                function workForProperty() {
                    ...
                    console.log('amount,extra_price, booking_price', amount, extra_price, booking_price);
                     total_service_booking_amount = amount + extra_price + booking_price+extra_cylinder_price+extra_house_key_price;
    ...
    function blsd_display_custom_text_cart($item_data, $cart_item) {
			...
			if($cart_item['deal_type'] == 'home_lockout'){
				...
				if (isset($cart_item['extra_deadbolt_cylinder_price']) && !empty($cart_item['extra_deadbolt_cylinder_price'])) {
					$value_total=$cart_item['extra_deadbolt_cylinder']*$cart_item['extra_per_deadbolt_cylinders_price'];
					  $item_data[] = array(
						'key' => __($cart_item['extra_deadbolt_cylinder'].' (extra cylinder)'.' X '.get_woocommerce_currency_symbol().$cart_item['extra_per_deadbolt_cylinders_price'].' (per cylinder) =', 'iconic'),
        ...
        if(isset($cart_item['no_of_keys_want_to_made'])){    
            $item_data[] = array(
                'key' => __('How many additional car keys do you want made?', 'iconic'),
    ...
    function blsd_add_custom_data_order_items($item, $cart_item_key, $values, $order) {
        ...
        if(!empty($values['no_of_keys_want_to_made'])){
            $item->add_meta_data(__('How many additional car keys do you want made?', 'blsd'), $values['no_of_keys_want_to_made']);
		...
		if(!empty($values['extra_deadbolt_cylinder_price'])){
            ...
            $item->add_meta_data(__($values['extra_deadbolt_cylinder'].' (extra cylinder)'.' X '.$values['extra_per_deadbolt_cylinders_price'].' = ', 'blsd'), $value_total);
        }
