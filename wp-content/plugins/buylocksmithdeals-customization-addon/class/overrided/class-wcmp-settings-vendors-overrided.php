...
class WCMp_Settings_WCMp_Vendors_overrided extends WP_List_Table {
    ...
    public function get_columns() {
    	$columns = [
    		...
    		'assing_products' => __( 'Assign Products', 'dc-woocommerce-multi-vendor' ),
    ...
    function column_username( $item ) {
    	$name_link = sprintf('?page=%s&action=%s&ID=%s', 'vendors', 'edit', $item['ID']);
    	$action = 'bulk-' . $this->_args['plural'];
    	$actions = array(
			'edit'=> sprintf('<a href="' . $name_link . '">' . __( 'Edit', 'dc-woocommerce-multi-vendor' ) . '</a>'),
			'delete'=> sprintf('<a href="?page=%s&action=%s&ID=%s&_wpnonce=%s">' . __( 'Delete', 'dc-woocommerce-multi-vendor' ) . '</a>', 'vendors', 'delete', $item['ID'], wp_create_nonce($action) ),
    ...
    function column_assing_products( $item ) {
       
        $id = $item['ID'];
    	return '<a href="'.home_url().'/wp-admin/admin.php?page=assign-product-to-vendors&vendor='.$id.'"><button type="button" class="assign_product">Assign</button></a>';
    }
