...
class BuyLockSmithDealsCustomizationFrontEnd {
    ...
    public function init_hooks() {
		...
		function blsd_wc_terms_and_conditions_checkbox_text() {
			...
			echo wp_kses_post( self::blsd_wc_replace_policy_page_link_placeholders( $text ) );
		}
		
		function blsd_wc_replace_policy_page_link_placeholders( $text ) {
			...
			$terms_link      = $terms_page_id ? '<a href="' . esc_url( get_permalink( $terms_page_id ) ) . '" class="woocommerce-terms-and-conditions-link" target="_blank">' . __( 'Terms of service', 'woocommerce' ) . '</a>' : __( 'Terms of service1', 'woocommerce' );
		...
		/* function blsd_update_checkout_terms_condition($text){
			$text=sprintf( __( 'I have read and agree to the website %s and %s', 'woocommerce' ), '[terms],[privacy_policy]' );
			return $text;
			
		} */
		function blsd_change_vendor_registration_button_text($button_text){
			if($button_text == 'Apply to become a vendor'){
				$button_text='Login To Your Dashboard';
			}
			return $button_text;
		}
		function blsd_change_vendor_header_registration_text($header_text){
			$header_text='Locksmith Login';
			return $header_text;
		}
		function blsd_gforms_confirmation_dynamic_redirect( $confirmation, $form, $entry, $ajax ) {
			$key='buylocksmithdeals';
			$code=urldecode($_REQUEST['token']);
			$user_login=BuyLockSmithDealsCustomizationAddon::code_decrypt_static($code, $key);
			...
			$save_from_token=get_user_meta($user_id,'save_from_token_another',true);
			if($save_from_token == 'yes'){
				$confirmation = '<span class="fail_message">You have already used this token.</span>';
			}
			else{
				$confirmation = '<span class="success_message">Thanks for contacting us! We will get in touch with you shortly.</span>';
			}
			$confirmation .='<style>
			span.success_message{
				...
				border-bottom:2px solid #77c84e;
				padding-top: 15px;
			}
			span.fail_message{
				text-align: center;
			...
			$current_url= home_url().'/locksmith-sign-up/?token='.$code;
			$url          = $current_url;
			GFCommon::log_debug( __METHOD__ . '(): Redirect to URL: ' . $url );
			$confirmation .= "<script type=\"text/javascript\">window.open('$url');</script>";
			return $confirmation;
		}

		function pending_vendor_message($message){
			$message='Congratulations, your email is now registered in Buy Locksmith Deals. You are one step closer to having your store go live. Please check your email for further instructions.';
			return $message;
		}
	   
		  function after_registration_form_submission( $entry, $form ){
			...
			$save_from_token=get_user_meta($user_id,'save_from_token',true);
			if($save_from_token != 'yes'){
			$new_fields=[];
			$new_fields_email=[];
			foreach ( $form['fields'] as $field ) {
				...
				$label=$field->label;
				if($label == ''){ $label= $field->placeholder;}
				
				$string = str_replace(' ', '_', $label);
				$field_key=preg_replace('/[^A-Za-z0-9\_]/', '', $string);
				if($field->visibility == 'visible' && $field->type != 'section'){
					if ( is_array( $inputs ) ) {
						$values=[];
						foreach ( $inputs as $input ) {
							$value = rgar( $entry, (string) $input['id'] );
							if($value !=''){
								$values[$input['label']]=$value;
							}
						}
						$new_fields[$field_key]=$values;
						$new_fields_email[$label]=$values;
					} else {
						$value = rgar( $entry, (string) $field->id );
						$new_fields[$field_key]=$value;
						$new_fields_email[$label]=$value;
					}
				}
			...
			update_user_meta($user_id,'save_from_token','yes');
			
			/*****************************/
			$vendor_info=get_user_by('ID',$user_id);
			$to = get_option( 'admin_email' ); 
			$admin = get_user_by('email',$to);
			$admin_name='';
			if($admin)
			{
				$admin_name= $admin->display_name;
			}
			$subject = 'New Locksmith registration';
            $message = 'Locksmith Details are as follows: <br><br/>';
            $message .= '<br>';
			$message .= 'Locksmith Name: ' .$vendor_info->display_name.'<br>';
			$message .= '<br>';
			$message .= 'Locksmith Email: ' . $vendor_info->user_email.'<br>';
			$message .= '<br>';
			if(!empty($new_fields_email)){
				foreach($new_fields_email as $email_key=>$email_value){
					if(is_array( $email_value )){
						$array_values='';
						foreach($email_value as $key=>$value){
							$array_values .= $key.': ' . $value.'<br>';
						}
						$message .= $email_key.': <br>';
						$message .= $array_values;
						$message .= '<br>';
					}
					else{
						$message .= $email_key.': ' . $email_value.'<br>';
						$message .= '<br>';
					}
				}
			}
			$message .= 'Please Check all details and approve from admin panel.';
			$message .= '<br>';
            $mailer = WC()->mailer();
			...
			$mailer->send( $recipient, $subject, $content, $headers, $attachments ); 
				...
				update_user_meta($user_id,'save_from_token_another','yes');
			}
          }
	   ...
	   function blsd_vendor_product($arguments){
		  $vendor_id= $arguments['vendor_id'];
		   $args = array('post_type' => 'product',
			...
			if (!empty($products)) {
				$html .= '<link rel="stylesheet" href="'.BUYLOCKSMITH_DEALS_ASSETS_PATH.'css/owl.carousel.min.css"> 
							<link rel="stylesheet" href="'.BUYLOCKSMITH_DEALS_ASSETS_PATH.'css/slider-style.css">';
							
			$html .='<div class="owl-slider">
						...
						foreach ($products as $product) {	
							...
							if (count($handle->get_attributes()) == 0) { //Condition for skip variation products
								...
								$html .='<div class="item">
									<div class="prd-image">
										<img src="' . $images . '" data-id="' . $product->ID . '">
										<div class="buy-now-btn"><a href="' . get_permalink($product->ID) . '" target="_blank">Buy Now</a></div>
									</div>
									<div class="content">
						...
						$html .='<script src="'.BUYLOCKSMITH_DEALS_ASSETS_PATH.'js/jquery.min.js"></script>
					<script src="'.BUYLOCKSMITH_DEALS_ASSETS_PATH.'js/owl.carousel.min.js"></script>';
				$html .='<script>
...
.mobile_btn_buy_now {
    ...
    function blsd_vendor_new_order($order_id){
        ...
        foreach($sub_orders as $sorder){
           $suborder_id=$sorder->ID;
			...
			$vendor_phone = get_user_meta($suborder_authorid, '_vendor_mobile_phone', true);
            if(!empty($vendor_phone)){
				if(empty($vendor_phone_prefix)){ $vendor_phone_prefix='+1'; }
            $phone_no=$vendor_phone_prefix.$vendor_phone;
            $message='You have just sold a deal on Buy Locksmith Deals. Check your email for more details. Ref Order #'.$suborder_id;
   ...
   function blsd_add_js_frontend(){
			...
			/* if (is_user_logged_in()) {
					...
					if ( in_array( 'dc_vendor', $user_roles ) ) {
						...
						if ($user_meta_post_id) {
							...
							$link_output .= '<a href="' . $vendor->permalink . '" class="vendor-breadcrumbs vendor-breadcrumbs-logo"><img style="height: 36px;" src="' . $get_user_profile_image_url . '"></a>';
						} 
					}
					else{
    ...
    function blsd_frontend_before_add_to_cart_btn() {
		...
		/***********Car Key Programming - CAR ***********/
		$car_programming_fee=  get_post_meta($post_id, 'car_programming_fee', true);
		$inshop_car_programming_fees=  get_post_meta($post_id, 'inshop_car_programming_fees', true);
		...
		if($inshop_car_programming_fees ==''){
			$inshop_car_programming_fees =0;    
		}
		$car_key_look_like = get_post_meta($post_parent_id, 'car_key_look_like', true);
		$is_in_shop_pricing_cp = get_post_meta($post_parent_id, 'is_in_shop_pricing_cp', true);
		...
		//inshop pricing value
		$inshop_double_sided_car_key_price=get_post_meta($post_id, 'inshop_double_sided_car_key_price', true);
		$inshop_high_security_car_key_price=get_post_meta($post_id, 'inshop_high_security_car_key_price', true);
		$inshop_tibbe_car_key_price=get_post_meta($post_id, 'inshop_tibbe_car_key_price', true);
		$inshop_car_key_programmed_price=get_post_meta($post_id, 'inshop_car_key_programmed_price', true);
		$inshop_vats_car_key_cut_price=get_post_meta($post_id, 'inshop_vats_car_key_cut_price', true);
		$inshop_vats_car_key_programmed_price=get_post_meta($post_id, 'inshop_vats_car_key_programmed_price', true);
		...
		$quantity_of_cars_unlocks = get_post_meta( $post_id, 'quantity_of_cars_unlocks', true );
		//$price_of_additional_car_unlocks = get_post_meta( $post_id, 'price_of_additional_car_unlocks', true );
		/*****************************/
		...
		/****************IN SHOP PRICING******************/
        $in_shop_pricing_cost = get_post_meta($post_id, '_wc_booking_in_shop_pricing_cost', true);
        $in_shop_duplication_price_cost = get_post_meta($post_id, '_wc_in_shop_key_duplication_price_cost', true);
		/*************************************************/
		
		/*****Home Lockout Service - PROPERTY****/
		$quantity_of_door_locks = get_post_meta( $post_parent_id, 'quantity_of_door_locks', true );
		
		$quantity_of_door_locks_unlocks = get_post_meta( $post_id, 'quantity_of_door_locks_unlocks', true );
		//$price_of_additional_door_locks_unlocks = get_post_meta( $post_id, 'price_of_additional_door_locks_unlocks', true );
		/****************************/
        ...
        $additional_customer_fresh_install_deadbolt_price = get_post_meta($post_id, 'customer_fresh_install_deadbolt', true);
        $additional_customer_replaced_deadbolt_price = get_post_meta($post_id, 'customer_replaced_deadbolt', true);
        $additional_locksmith_fresh_install_deadbolt_price = get_post_meta($post_id, 'locksmith_fresh_install_deadbolt', true);
        $additional_locksmith_replaced_deadbolt_price = get_post_meta($post_id, 'locksmith_replaced_deadbolt', true);
        ...
        echo '<input type="hidden" name="deal_type" id="deal_type" value="'.$deal_type.'">';
		...
		if($deal_type == 'lock_rekeying' || $deal_type == 'home_lockout' || $deal_type == 'deadbolt'){
			...
			if($deal_type == 'deadbolt'){
				...
				echo '<input type="hidden" name="deadbolt_extra_quantity" id="deadbolt_extra_quantity" value="">';
				echo '<input type="hidden" name="deadbolt_extra_per_price" id="deadbolt_extra_per_price" value="">';
				echo '<input type="hidden" name="extra_deadbolt" id="extra_deadbolt" value="">';
				...
				if($quantity_of_locks_install == 'yes'){
					...
					. $option
					. '</select> <span class="chat_support" id="small_text_deadbolt">'
					. '</span>'
					. '<input type="hidden" name="extra_deadbolt_cylinder_price" id="extra_deadbolt_cylinder_price" class="extra_deadbolt_cylinder_price" value="">'
					. '<input type="hidden" name="extra_deadbolt_cylinder" id="extra_deadbolt_cylinder" class="extra_deadbolt_cylinder" value="">'
					. '<input type="hidden" name="extra_per_deadbolt_cylinders_price" id="extra_per_deadbolt_cylinders_price" class="extra_per_deadbolt_cylinders_price" value="">'
					.'</div>';
				}
			}
				...
				if ($quantity_of_locks == 'yes') {
					...
					echo '<div class="options_show"><label> Choose Quantity Of Lock Rekeying</label>';
					echo '<select name="quantity_of_locks_to_rekey" id="quantity_of_locks_to_rekey">'
					. $option
					. '</select> <span class="chat_support chat_support_hide">'
					. '*This deal includes up to '.$cylinders_included.' cylinder rekeyed. Let us know how many locks you need rekeyed.'
					. '</span>'
					...
					. '<input type="hidden" name="inshop_extra_per_cylinders_price" id="inshop_extra_per_cylinders_price" class="inshop_extra_per_cylinders_price" value="">'
					. '</div>'
					. '</div>';
				...
				if($house_keys_included == 'yes'){
					$option = '<option value="">Choose Quantity Of Duplicate Keys</option>';
					for ($i = 1; $i <= 20; $i++) {
						$option .= '<option value="' . $i . '" >' . $i . '</option>';
					}
					echo '<div class="options_show"><label> Choose Quantity Of Duplicate Keys</label>';
					echo '<select name="quantity_of_extra_house_keys" id="quantity_of_extra_house_keys">'
					. $option
					. '</select> <span class="chat_support chat_support_hide">'
					. '*Deals includes ' . $house_keys_included_deal . ' keys. Select how many extra keys you would like.'
					. '</span>'
					. '<input type="hidden" name="extra_house_key_price" id="extra_house_key_price" class="extra_house_key_price" value="">'
					. '<input type="hidden" name="extra_house_key" id="extra_house_key" class="extra_house_key" value="">'
					. '<input type="hidden" name="extra_per_house_key_price" id="extra_per_house_key_price" class="extra_per_house_key_price" value="">'
					. '<input type="hidden" name="inshop_extra_per_house_key_price" id="inshop_extra_per_house_key_price" class="inshop_extra_per_house_key_price" value="">'
					. '</div>';
				}
			}
			else if($deal_type == 'home_lockout'){
				if($quantity_of_door_locks == 'yes'){
					echo '<div class="options_show"><label> Door Locks to unlock</label>';
					echo '<input type="hidden" name="quantity_of_door_locks_unlocks" id="quantity_of_door_locks_unlocks" value="'.$quantity_of_door_locks_unlocks.'">';
					echo '<span class="chat_support">'
					. '*The deal includes 1 house unlock.'
					. '</span>'
					. '</div>';
				}
		...
		else if($deal_type == 'car_lockout' || $deal_type == 'car_key_programming'){
			...
			if($deal_type == 'car_lockout'){
				if($quantity_of_car_unlocks == 'yes'){
					echo '<div class="options_show"><label> Car to unlock</label>';
					echo '<input type="hidden" name="quantity_of_cars_unlocks" id="quantity_of_cars_unlocks" value="'.$quantity_of_cars_unlocks.'">';
					echo '<span class="chat_support">'
					. '*This deal includes 1 car unlock.'
					. '</span>'
					. '</div>';
				}
			}
				...
				if($car_key_look_like == 'yes'){
					...
					echo '<input type="hidden" name="car_key_acutal_type" id="car_key_acutal_type" value="">';
					echo '<input type="hidden" name="car_key_price" id="car_key_price" value="">';
					echo '<input type="hidden" name="in_shop_cp_price" id="in_shop_cp_price" value="">';
					echo '<input type="hidden" name="inshop_car_programming_fees" id="inshop_car_programming_fees" value="">';
					echo '<input type="hidden" name="inshop_car_key_price" id="inshop_car_key_price" value="">';
					echo '<input type="hidden" name="is_in_shop_pricing_cp" id="is_in_shop_pricing_cp" value="'.$is_in_shop_pricing_cp.'">';

					if(!empty($file_edge_cut)){
						echo '<div class="car-key-img"><span class="car_key_img" data-id="double_sided"><img src="'.$target_dir_img.$file_edge_cut.'" height="50" width="50"></span></div>';
					}
					else{
						echo '<div class="car-key-img car_key_name"><span class="car_key_img" data-id="double_sided">Double-Sided</span></div>';
				   }
				...
				if($want_more_than_one_key == 'yes'){
				  ...
				  $option = '<option value="">Choose number of keys</option>';
					...
					echo '<div id="keys_count" class="options_show"><label> How many additional car keys do you want made?  </label>'
					. '<select name="no_of_keys_want_to_made" id="no_of_keys_want_to_made">'
					. $option
					. '</select><span class="chat_support chat_support_hide">*This deal includes the cutting and programming of 1 car key. Let us know how many car keys you need.</span></div>';
       ...
       <script>
			...
			function calculate_deal_pricing_deadbolt(){
				...
				if (service_location_price >= 0 && service_location_val != '' && service_location_label == 'Customer’s Location') {
					...
					var total_service_booking_amount = parseFloat(amount + total + booking_price + deadbolt_install_price + extra_deadbolt_cylinder_price);
					...
					var subtotal_html = '<p class="price">Subtotal <span class="amount"><span class="currency"><?php echo $currenct_symbol; ?></span>' + ( parseFloat(total_service_booking_amount)).toFixed(2) + '</span></p>';
					...
					if (Number.isNaN(booking_price)) {
                     ...
                     var total_service_booking_amount=parseFloat(booking_price+deadbolt_install_price+extra_deadbolt_cylinder_price);
			...
			function calculate_deal_pricing_locks_rekeying(){
				var service_location_val = jQuery('.wc-pao-addon-select').val();
                var service_location_price = jQuery('.wc-pao-addon-select').find(':selected').attr('data-price');
				var service_location_label = jQuery('.wc-pao-addon-select').find(':selected').attr('data-label');
				if (Number.isNaN(service_location_price)) {
				...
				if (service_location_price >= 0 && service_location_val != '' && service_location_label == 'Customer’s Location') {
					...
					service_location_price = parseFloat(service_location_price);
					var total_amount = parseFloat(amount + total);
					var total_service_booking_amount = parseFloat(amount + total + booking_price  +extra_cylinder_price  +extra_house_key_price);
					jQuery('.main_loader').removeClass('disabled');
					var html = '<span class="currency"><?php echo $currenct_symbol; ?></span>' + total_amount.toFixed(2);
					jQuery('.product-addon-totals .wc-pao-col2 .amount').html('');
					jQuery('.product-addon-totals .wc-pao-col2 .amount').html(html);
				...
				}else if(in_shop_duplication_price_cost > 0 || in_shop_pricing_cost > 0){
					jQuery('.chat_support_hide').hide();
					//in shop caluculation
					var quantity_of_locks_to_rekey=parseInt(jQuery('#quantity_of_locks_to_rekey').val());
					if (Number.isNaN(quantity_of_locks_to_rekey)) {
					    quantity_of_locks_to_rekey = 0;
					}
					if (in_shop_pricing_cost > 0) {
						console.log('inshop price', in_shop_pricing_cost);
						var inshop_cylinders_amt = parseInt(quantity_of_locks_to_rekey*in_shop_pricing_cost);
					}else{
						var inshop_cylinders_amt = 0;
					}


					//in shop caluculation
					var quantity_of_extra_house_keys=parseInt(jQuery('#quantity_of_extra_house_keys').val());
					if (Number.isNaN(quantity_of_extra_house_keys)) {
					    quantity_of_extra_house_keys = 0;
					}
					if (in_shop_duplication_price_cost > 0) {
					     console.log('inshop price', in_shop_duplication_price_cost);
					     var inshop_duplicate_amt = parseInt(quantity_of_extra_house_keys*in_shop_duplication_price_cost);
					}else{
					     var inshop_duplicate_amt = 0;
					}
                    ...
                    var total_service_booking_amount=parseFloat(inshop_cylinders_amt+inshop_duplicate_amt);
                    var discount_percentage=parseFloat('<?php echo $discount_on_deal; ?>');
					if (Number.isNaN(discount_percentage)) {
						discount_percentage = 0;
					}
                    ...
                    var total_service_booking_amount=parseFloat(booking_price+extra_cylinder_price+extra_house_key_price);
                    var discount_percentage=parseFloat('<?php echo $discount_on_deal; ?>');
					if (Number.isNaN(discount_percentage)) {
						discount_percentage = 0;
					}
			...
			function calculate_deal_pricing_home_lockout(){
				var service_location_val = jQuery('.wc-pao-addon-select').val();
                var service_location_price = jQuery('.wc-pao-addon-select').find(':selected').attr('data-price');
				var service_location_label = jQuery('.wc-pao-addon-select').find(':selected').attr('data-label');
				if (Number.isNaN(service_location_price)) {
				...
				if (service_location_price >= 0 && service_location_val != '' && service_location_label == 'Customer’s Location') {
					...
					service_location_price = parseFloat(service_location_price);
					var total_amount = parseFloat(amount + total);
					console.log('home_lockout',amount,total,booking_price);
					var total_service_booking_amount = parseFloat(amount + total + booking_price);
                    jQuery('.main_loader').removeClass('disabled');
					var html = '<span class="currency"><?php echo $currenct_symbol; ?></span>' + total_amount.toFixed(2);
					jQuery('.product-addon-totals .wc-pao-col2 .amount').html('');
					jQuery('.product-addon-totals .wc-pao-col2 .amount').html(html);
                    ...
                    var total_service_booking_amount=parseFloat(booking_price);
                    var discount_percentage=parseFloat('<?php echo $discount_on_deal; ?>');
					if (Number.isNaN(discount_percentage)) {
						discount_percentage = 0;
					}
			...
			function calculate_deal_pricing_car_lockout(){
				var service_location_val = jQuery('.wc-pao-addon-select').val();
                var service_location_price = jQuery('.wc-pao-addon-select').find(':selected').attr('data-price');
				var service_location_label = jQuery('.wc-pao-addon-select').find(':selected').attr('data-label');
				if (Number.isNaN(service_location_price)) {
			...
			function select_location_by_user()
			{
				var service_location_val = jQuery('.wc-pao-addon-select').val();
                var service_location_price = jQuery('.wc-pao-addon-select').find(':selected').attr('data-price');
				var service_location_label = jQuery('.wc-pao-addon-select').find(':selected').attr('data-label');
			...
			function calculate_deal_pricing_car_key_programming(){
				var service_location_val = jQuery('.wc-pao-addon-select').val();
                var service_location_price = jQuery('.wc-pao-addon-select').find(':selected').attr('data-price');
				var service_location_label = jQuery('.wc-pao-addon-select').find(':selected').attr('data-label');
				if (Number.isNaN(service_location_price)) {
				...
				var inshop_car_key_price = parseFloat(jQuery('#inshop_car_key_price').val());
				var inshop_car_programming_fees = parseFloat(jQuery('#inshop_car_programming_fees').val());
				if (service_location_price >= 0 && service_location_val != '' && service_location_label == 'Customer’s Location') {
					jQuery('.chat_support_hide').show();
					var amount = parseFloat(jQuery('#amount').val());
					if (Number.isNaN(amount)) {
					...
					service_location_price = parseFloat(service_location_price);
					var total_amount = parseFloat(amount + total);
					var total_service_booking_amount = parseFloat(amount + total + booking_price +car_key_price + key_cost +car_programming_fees);
					jQuery('.main_loader').removeClass('disabled');
					var html = '<span class="currency"><?php echo $currenct_symbol; ?></span>' + total_amount.toFixed(2);
					jQuery('.product-addon-totals .wc-pao-col2 .amount').html('');
					jQuery('.product-addon-totals .wc-pao-col2 .amount').html(html);
				...
				else if(inshop_car_programming_fees > 0 && inshop_car_key_price > 0){
					jQuery('.chat_support_hide').hide();
					jQuery('.main_loader').removeClass('disabled');
					var booking_price = parseFloat(jQuery('#booking_price').val());
					var car_key_price = parseFloat(jQuery('#inshop_car_key_price').val());
					var car_programming_fees = parseFloat(jQuery('#inshop_car_programming_fees').val());
					var no_of_keys_want_to_made=jQuery('#no_of_keys_want_to_made').val();
					/* var cost_to_cut_additional_key=parseFloat('<?php echo $cost_to_cut_additional_key; ?>');
					var cost_to_program_additional_key=parseFloat('<?php echo $cost_to_program_additional_key; ?>');
					var total_key_cost=parseFloat(cost_to_cut_additional_key+cost_to_program_additional_key);
					...
					var key_cost=0; 
					 ...
					 if (Number.isNaN(key_cost)) {
						key_cost = 0;
                             ...
                             console.log(car_key_price,car_programming_fees);
                             var total_service_booking_amount=parseFloat(car_key_price+key_cost+car_programming_fees);
					...
					var key_cost=0; 
					 ...
					 if (Number.isNaN(key_cost)) {
						key_cost = 0;
                             ...
                             console.log(booking_price,car_key_price,key_cost,car_programming_fees);
                             var total_service_booking_amount=parseFloat(booking_price+car_key_price+key_cost+car_programming_fees);
            ...
            jQuery('.wc-bookings-booking-form').on('change', 'input, select:not("#wc-bookings-form-start-time, #wc-bookings-form-end-time")', function (e) {
                setTimeout(function () {
                    ...
                    if (booking_price != 0) {
						...
						var dealtype='<?php echo $deal_type; ?>';
						if(dealtype == 'deadbolt'){
							calculate_deal_pricing_deadbolt();
						}
						else if(dealtype == 'lock_rekeying' || dealtype == 'home_lockout'){
							if(dealtype == 'lock_rekeying'){
								calculate_deal_pricing_locks_rekeying();
							}
							else if(dealtype == 'home_lockout'){
								calculate_deal_pricing_home_lockout();
							}
							
						}
						else if(dealtype == 'car_lockout' || dealtype == 'car_key_programming'){
							if(dealtype == 'car_lockout'){
								calculate_deal_pricing_car_lockout();
							}
							else if(dealtype == 'car_key_programming'){
								calculate_deal_pricing_car_key_programming();
							}
			 ...
			 jQuery('.wc-pao-addon-select').change(function () {
                    ...
                    if (Number.isNaN(service_location_price)) {
					 ...
					 var service_location_label = jQuery(this).find(':selected').attr('data-label');
                    ...
                    if (service_location_price >= 0 && service_location_val != '' && service_location_label == 'Customer’s Location') {
						...
						var total_amount = parseFloat(amount + service_location_price);
			...
			jQuery(document).on('change', '.serviceable_car_year', function () {
                 ...
                 jQuery('#loading').removeClass('disabled');
                    jQuery.ajax({
                        ...
                        success: function (resultData) {
						   ...
						   var dealtype='<?php echo $deal_type; ?>';
						   if(dealtype == 'car_key_programming'){
			  ...
			  function calculate_programming_fee(){
					var car_key_type=jQuery('#car_key_acutal_type').val();
					var no_of_keys_want_to_made=jQuery('#no_of_keys_want_to_made').val();
                    calculate_car_key_amount(car_key_type,no_of_keys_want_to_made);
			  }
			...
			function calculate_amount_car_key_price(){
                ...
                //inshop calculation
                var selected_location = select_location_by_user();
                if (Number.isNaN(inshop_car_key_price)) {
                    inshop_car_key_price = 0;
                }
                if(selected_location == '1' && inshop_car_key_price > 0){
                	var amount = 0;
	                var booking_price = 0;
	                var booking_price_with_tax = 0;
                }
                /* var cost_to_cut_additional_key=parseFloat('<?php echo $cost_to_cut_additional_key; ?>');
                var cost_to_program_additional_key=parseFloat('<?php echo $cost_to_program_additional_key; ?>');
                var total_key_cost=parseFloat(cost_to_cut_additional_key+cost_to_program_additional_key);
                ...
                var key_cost=0;
				 ...
				 var extra_amount_property =  parseFloat(jQuery('#extra_amount_property').val());
				 if (Number.isNaN(extra_amount_property)) {
					extra_amount_property = 0;
				 }
				...
				if(extra_amount_property != ''){
					total = parseFloat(total + extra_amount_property);
				}
                ...
                console.log(amount, total, service_location_price, booking_price, booking_price_with_tax, car_key_price,key_cost,car_programming_fees,deadbolt_install_price,extra_deadbolt_cylinder_price);
                var total_amount = parseFloat(amount + total );
                var total_service_booking_amount = parseFloat(amount + total + booking_price + car_key_price + key_cost +car_programming_fees + deadbolt_install_price + extra_deadbolt_cylinder_price);
                ...
                var total_service_booking = parseFloat(amount + total + booking_price +car_key_price + key_cost + tax + car_programming_fees + deadbolt_install_price + extra_deadbolt_cylinder_price);
                setTimeout(function () {
                    var html = '<span class="currency"><?php echo $currenct_symbol; ?></span>' + total_amount.toFixed(2);
                    jQuery('.product-addon-totals .wc-pao-col2 .amount').html('');
                    jQuery('.product-addon-totals .wc-pao-col2 .amount').html(html);
			...
			/*******************CAR KEY PROGRAMMING*****************/
			function calculate_car_key_amount(car_key_type,no_of_keys_want_to_made){
					var additional_quantity=0;
					var additional_quantity_price=0;
					var inshop_quantity_price=0;
					
					if(car_key_type === 'double_sided'){
                       var additional_quantity='<?php echo $quantity_double_sided_car_key ?>';
                       var additional_quantity_price='<?php echo $additional_double_sided_car_key_price ?>';
                       var inshop_quantity_price='<?php echo $inshop_double_sided_car_key_price ?>';
                    }
                    else if(car_key_type === 'high_security'){
                        var additional_quantity='<?php echo $quantity_high_security_car_key ?>';
                        var additional_quantity_price='<?php echo $additional_high_security_car_key_price ?>';
                        var inshop_quantity_price='<?php echo $inshop_high_security_car_key_price ?>';
                    }
                    else if(car_key_type === 'tibbe'){
                        var additional_quantity='<?php echo $quantity_tibbe_car_key ?>';
                        var additional_quantity_price='<?php echo $additional_tibbe_car_key_price ?>';
                        var inshop_quantity_price='<?php echo $inshop_tibbe_car_key_price ?>';
                    }
                    else if(car_key_type === 'vats'){
                        var additional_quantity='<?php echo $quantity_vats_car_key_cut ?>';
                        var additional_quantity_price='<?php echo $additional_vats_car_key_cut_price ?>';
                        var inshop_quantity_price='<?php echo $inshop_vats_car_key_cut_price ?>';
                    }
					if(additional_quantity == ''){
						additional_quantity=0;
					}
					if(additional_quantity_price == ''){
						 additional_quantity_price=0;
					}
					if(inshop_quantity_price == ''){
						 inshop_quantity_price=0;
					}
					var total_car_key_price=0;
					var selected_location = select_location_by_user();
					if(no_of_keys_want_to_made != ''){
						if(additional_quantity > 0 && additional_quantity_price > 0 && selected_location == '0'){
							if(no_of_keys_want_to_made > additional_quantity ){
								var extra_key=parseFloat(no_of_keys_want_to_made-additional_quantity);
								total_car_key_price=parseFloat(extra_key*additional_quantity_price);
								jQuery('.chat_support_hide').show();
							}
						}else{
							total_car_key_price=parseFloat(no_of_keys_want_to_made*inshop_quantity_price);
							console.log("inshop car key price",total_car_key_price);
							jQuery('#inshop_car_key_price').val(total_car_key_price);
							jQuery('.chat_support_hide').hide();
						}
					}
					console.log("total car key price",total_car_key_price);
					
					jQuery('#car_key_price').val(total_car_key_price);
					...
					var vats_fees=0;
					var quantity_car_key_programmed ='<?php echo $quantity_car_key_programmed; ?>';
					var quantity_vats_car_key_programmed ='<?php echo $quantity_vats_car_key_programmed; ?>';
					var additional_car_key_programmed_price='<?php echo $additional_car_key_programmed_price; ?>';
					var inshop_car_key_programmed_price='<?php echo $inshop_car_key_programmed_price; ?>';
					var additional_vats_car_key_programmed_price='<?php echo $additional_vats_car_key_programmed_price; ?>';
					var inshop_vats_car_key_programmed_price='<?php echo $inshop_vats_car_key_programmed_price; ?>';
					console.log('car programming',car_programming);
					for (var i = 0; i < car_programming.length; i++) {
						if(car_programming[i] == 'Yes'){
							if(quantity_car_key_programmed !='' && additional_car_key_programmed_price !='' && selected_location == '0'){
								if(no_of_keys_want_to_made > quantity_car_key_programmed ){
									var extra_quantity_programmed=parseFloat(no_of_keys_want_to_made - quantity_car_key_programmed);
									var programmed_price=parseFloat(extra_quantity_programmed*additional_car_key_programmed_price);
									programming_fees=parseFloat(programming_fees+programmed_price);
									car_programming_fees=car_programming_fees+programming_fees;
								}
							}else{
								if(inshop_car_key_programmed_price > 0){
									var programmed_price=parseFloat(no_of_keys_want_to_made*inshop_car_key_programmed_price);
									programming_fees=parseFloat(programming_fees+programmed_price);
									car_programming_fees=car_programming_fees+programming_fees;
								}
							}
							...
							if(inshop_car_key_programmed_price > 0 && selected_location == '1'){
									var programmed_price=parseFloat(no_of_keys_want_to_made*inshop_car_key_programmed_price);
									programming_fees=parseFloat(programming_fees+programmed_price);
									car_programming_fees=car_programming_fees+programming_fees;
							}else{
						...
						if(car_type[i] == 'Vats'){
							...
							if(quantity_vats_car_key_programmed !='' && additional_vats_car_key_programmed_price !=''  && selected_location == '0'){
								if(no_of_keys_want_to_made > quantity_vats_car_key_programmed ){
									var extra_quantity_vats_programmed=parseFloat(no_of_keys_want_to_made - quantity_vats_car_key_programmed);
									var vats_programmed_price=parseFloat(extra_quantity_programmed*additional_vats_car_key_programmed_price);
									vats_fees=parseFloat(vats_fees+vats_programmed_price);
									car_programming_fees=parseFloat(car_programming_fees-programming_fees);
									car_programming_fees=parseFloat(car_programming_fees+vats_fees);
								}
							}else{
								if(inshop_vats_car_key_programmed_price > 0){
									var vats_programmed_price=parseFloat(no_of_keys_want_to_made*inshop_vats_car_key_programmed_price);
									vats_fees=parseFloat(vats_fees+vats_programmed_price);
									car_programming_fees=parseFloat(car_programming_fees-programming_fees);
									car_programming_fees=parseFloat(car_programming_fees+vats_fees);
								}
					...
					console.log(car_programming_fees,programming_fees,vats_fees);
					jQuery('#car_programming_fees').val(car_programming_fees); 
					jQuery('#inshop_car_programming_fees').val(car_programming_fees); 
			...
			jQuery('.car_key_img').click(function () {
                    ...
                    jQuery('#car_key_acutal_type').val(car_key_type);
					calculate_car_key_amount(car_key_type,no_of_keys_want_to_made);
				});
			/*******************END CAR KEY PROGRAMMING*****************/
            ...
            jQuery('.wc-pao-addon-select option:contains("None")').text('Choose service location');
                ...
                jQuery('.single_add_to_cart_button').click(function (e) {
                    ...
                    var totalmiles = jQuery('input[name="total_miles[]"]').map(function () {
                        if (parseFloat(this.value) > 0) {
							...
							console.log('ToTla_miles::',this.value );
							console.log('maximum_miles::',maximum_miles );
							
							if (Number.isNaN(maximum_miles)) {
							  maximum_miles = 0;
							}
							if(parseFloat(this.value) > maximum_miles){
								total_miles_val = 0;
							}
                        }
                        return this.value; // $(this).val()
                    }).get();
						console.log('total_miles_val::',total_miles_val);
                    ...
                    var no_of_keys_want_to_made=jQuery('#no_of_keys_want_to_made').val();
                    if(no_of_keys_want_to_made == ''){
                        alert('Please select how many car additional keys do you want made?');
                        redirect_flag = 1; 
                    }
				...
				/****************DEADBOLTS*************/
				function deadbolts_extra_quantity_locks_install(type_of_installation,who_is_supplying_deadbolts,quantity_locks_to_install){
					if(type_of_installation !='' && who_is_supplying_deadbolts !=''){
						if(type_of_installation == 'Fresh Install' && who_is_supplying_deadbolts == 'I am'){
							var quantity_deadbolt='<?php echo $quantity_customer_fresh_install_deadbolt ?>';
							var additional_deadbolt_price='<?php echo $additional_customer_fresh_install_deadbolt_price ?>';
						}
                        else if(type_of_installation == 'Fresh Install' && who_is_supplying_deadbolts == 'I want locksmith grade deadbolts'){
                            var quantity_deadbolt='<?php echo $quantity_locksmith_fresh_install_deadbolt ?>';
							var additional_deadbolt_price='<?php echo $additional_locksmith_fresh_install_deadbolt_price ?>';
                        }
                        else if(type_of_installation == 'Replacement' && who_is_supplying_deadbolts == 'I am'){
							var quantity_deadbolt='<?php echo $quantity_customer_replaced_deadbolt ?>';
							var additional_deadbolt_price='<?php echo $additional_customer_replaced_deadbolt_price ?>';
                        }
                        else if(type_of_installation == 'Replacement' && who_is_supplying_deadbolts == 'I want locksmith grade deadbolts'){
                            var quantity_deadbolt='<?php echo $quantity_locksmith_replaced_deadbolt ?>';
							var additional_deadbolt_price='<?php echo $additional_locksmith_replaced_deadbolt_price ?>';
                        }
						var deadbolt_text='*This deal includes the installation of '+quantity_deadbolt+' deadbolt. Let us know how many deadbolts you need installed.';
						jQuery('#small_text_deadbolt').text(deadbolt_text);
						if(quantity_locks_to_install !=''){
							if(quantity_locks_to_install > quantity_deadbolt){
								var extra_deadbolt_cylinders=parseInt(quantity_locks_to_install - quantity_deadbolt);
								var extra_deadbolt_cylinders_price=parseInt(extra_deadbolt_cylinders*additional_deadbolt_price);
								calculate_final_deadbolt_amount(extra_deadbolt_cylinders,additional_deadbolt_price);
							}
							else{
							 calculate_final_deadbolt_amount(0,additional_deadbolt_price);   
							}
						}
				...
				jQuery('#quantity_locks_to_install').change(function(){
					var type_of_installation=jQuery('#type_of_installation').val();
                    var who_is_supplying_deadbolts=jQuery('#who_is_supplying_deadbolts').val();
					var quantity_locks_to_install=jQuery('#quantity_locks_to_install').val();
					deadbolts_extra_quantity_locks_install(type_of_installation,who_is_supplying_deadbolts,quantity_locks_to_install);
				});
				...
				function calculate_final_deadbolt_amount(extra_deadbolt_cylinders,extra_per_deadbolt_cylinders_price){
					var service_location_val = jQuery('.wc-pao-addon-select').val();
                    var service_location_price = jQuery('.wc-pao-addon-select').find(':selected').attr('data-price');
                    if (Number.isNaN(service_location_price)) {
                         service_location_price = 0;
                    ...
                    if (element.classList.contains("property")) {
                        ...
                        var extra_deadbolt_cylinder_price=extra_deadbolt_cylinders*extra_per_deadbolt_cylinders_price;
                        jQuery('#extra_deadbolt_cylinder_price').val(extra_deadbolt_cylinder_price);
                        jQuery('#extra_deadbolt_cylinder').val(extra_deadbolt_cylinders);
                        jQuery('#extra_per_deadbolt_cylinders_price').val(extra_per_deadbolt_cylinders_price);
                        console.log('dead::',amount, total, service_location_price, booking_price, booking_price_with_tax,extra_deadbolt_cylinder_price);
                        var total_amount = parseFloat(total + service_location_price);
                        if(service_location_price >0){
                          var total_service_booking_amount = parseFloat( total + service_location_price + booking_price+extra_deadbolt_cylinder_price+deadbolt_install_price);
                          console.log('total_service_booking_amount',total_service_booking_amount);
                            if (booking_price_with_tax === '') {
                                var tax=0;
                            } else {
                            ...
                            var total_service_booking = parseFloat( total + service_location_price + booking_price +extra_deadbolt_cylinder_price + deadbolt_install_price +tax);

                        }
                        else{
                           var total_service_booking_amount = parseFloat(booking_price+extra_deadbolt_cylinder_price + deadbolt_install_price );
                           var total_service_booking = parseFloat(booking_price +extra_deadbolt_cylinder_price + deadbolt_install_price);
                        }
				...
				jQuery('#quantity_of_extra_house_keys').change(function(){
					//service charge 
                    var quantity_of_extra_house_keys=parseInt(jQuery('#quantity_of_extra_house_keys').val());
                    var included_house_key='<?php echo $house_keys_included_deal; ?>';
					console.log('Run Duplicate quantity');
                    var extra_per_house_key_price='<?php echo $extra_per_house_keys_included_price; ?>';

                    //in-shop price
                    var in_shop_duplication_price_cost = '<?php echo $in_shop_duplication_price_cost; ?>';

                    console.log(included_house_key+'+'+quantity_of_extra_house_keys);
                    if(included_house_key < quantity_of_extra_house_keys){
                        var extra_house_keys=parseInt(quantity_of_extra_house_keys - included_house_key);
                        var extra_cylinders_price=parseInt(extra_house_keys*extra_per_house_key_price);
                        console.log('extra_house_key2', extra_house_keys);
                        console.log('extra_cylinders_price2', extra_cylinders_price);
                        
                        calculate_house_key_final_amount(extra_house_keys,extra_per_house_key_price);
                        
                        
                    }
                    else{
                     calculate_house_key_final_amount(0,extra_per_house_key_price);   
                    }
                });
				function calculate_house_key_final_amount(extra_house_keys,extra_per_house_key_price){
                    ...
                    jQuery('#inshop_extra_per_house_key_price').val(inshop_duplicate_amt);
                    var inshop_extra_cylinder_price= parseFloat(jQuery('#inshop_extra_per_cylinders_price').val());
					if (Number.isNaN(inshop_extra_cylinder_price)) {
                        inshop_extra_cylinder_price = 0;
                    }

                    console.log('Inshop Duplicate key',inshop_duplicate_amt);
						...
						var extra_house_key_price=extra_house_keys*extra_per_house_key_price;
                        jQuery('#extra_house_key_price').val(extra_house_key_price);
                        jQuery('#extra_house_key').val(extra_house_keys);
                        jQuery('#extra_per_house_key_price').val(extra_per_house_key_price);
                        console.log(amount, total, service_location_price, booking_price, booking_price_with_tax,extra_house_key_price,extra_cylinder_price);
                        var total_amount = parseFloat(total + service_location_price);
                        if(service_location_price >0){
                          var total_service_booking_amount = parseFloat( total + service_location_price + booking_price+extra_house_key_price+extra_cylinder_price);
                          console.log('total_service_booking_amount',total_service_booking_amount);
                            if (booking_price_with_tax === '') {
                                var tax=0;
                            } else {
                            ...
                            var total_service_booking = parseFloat( total + service_location_price + booking_price +extra_house_key_price +tax);

                        }else if (in_shop_duplication_price_cost > 0) {
                           var total_service_booking_amount = parseFloat(inshop_duplicate_amt +inshop_extra_cylinder_price);
                           var total_service_booking = parseFloat(inshop_duplicate_amt +inshop_extra_cylinder_price);
                        }
                        else{
                           var total_service_booking_amount = parseFloat(booking_price+extra_house_key_price+extra_cylinder_price);
                           var total_service_booking = parseFloat(booking_price +extra_house_key_price+extra_cylinder_price);
                        }
                ...
                function calculate_final_amount(extra_cylinders,extra_per_cylinders_price){
                    ...
                    if (in_shop_pricing_cost > 0) {
                    	console.log('inshop price', in_shop_pricing_cost);
                    	var inshop_cylinders_amt = quantity_of_locks_to_rekey*in_shop_pricing_cost;
                    }else{
                    	var inshop_cylinders_amt = 0;
                    }
                    jQuery('#inshop_extra_per_cylinders_price').val(inshop_cylinders_amt);
                    var inshop_extra_key_price= parseFloat(jQuery('#inshop_extra_per_house_key_price').val());
					if (Number.isNaN(inshop_extra_key_price)) {
                        inshop_extra_key_price = 0;
                    }

                    console.log('Inshop cylinders key',inshop_cylinders_amt);
                        ...
                        var total_amount = parseFloat(total + service_location_price);
                        if(service_location_price >0){
                          var total_service_booking_amount = parseFloat( total + service_location_price + booking_price+extra_cylinder_price+extra_house_key_price);
                          console.log('total_service_booking_amount',total_service_booking_amount);
                            if (booking_price_with_tax === '') {
                                var tax=0;
                            } else {
                            ...
                            var total_service_booking = parseFloat( total + service_location_price + booking_price +extra_cylinder_price +extra_house_key_price +tax);

                        }else if (in_shop_pricing_cost > 0) {
                        	var total_service_booking_amount = parseFloat(inshop_cylinders_amt+inshop_extra_key_price);
                           var total_service_booking = parseFloat(inshop_cylinders_amt+inshop_extra_key_price);
                        }
                        else{
                           var total_service_booking_amount = parseFloat(booking_price+extra_cylinder_price+extra_house_key_price);
                           var total_service_booking = parseFloat(booking_price +extra_cylinder_price+extra_house_key_price);
                        }
    ...
    function blsd_add_custom_data_to_cart_item($cart_item_data, $product_id, $variation_id) {
        ...
        $total_miles = $_POST['total_miles']; //filter_input( INPUT_POST, 'total_miles' );
        $maximum_miles = $_POST['maximum_miles']; //filter_input( INPUT_POST, 'maximum_miles' );
        $default_miles = $_POST['default_miles']; //filter_input( INPUT_POST, 'default_miles' );
        $extra_permile_price = $_POST['extra_permile_price']; //filter_input( INPUT_POST, 'extra_permile_price' );
        $extra_amount = $_POST['extra_amount']; //filter_input( INPUT_POST, 'extra_amount' );
        ...
        for ($i = 0; $i < $total; $i++) {
			...
			$total_miles_array[]=['total_miles'=>$total_miles[$i],'maximum_miles'=>$maximum_miles,'default_miles'=>$default_miles,'extra_permile_price'=>$extra_permile_price,'extra_amount'=>$extra_amount[$i]];
		}
		...
		$deal_type = filter_input(INPUT_POST, 'deal_type');
        ...
        $total_miles_property = filter_input(INPUT_POST, 'total_miles_property');
        $extra_amount_property = filter_input(INPUT_POST, 'extra_amount_property');
		/*******************/
        
		/********Car key Programming********/
		$car_key_type = filter_input(INPUT_POST, 'car_key_type');
        $car_key_price = filter_input(INPUT_POST, 'car_key_price');
        $inshop_car_key_price = filter_input(INPUT_POST, 'inshop_car_key_price');
        // $inshop_car_key_price = filter_input(INPUT_POST, 'car_key_price');
        $car_programming_fees = filter_input(INPUT_POST, 'cprogramming_fees');
        // $inshop_car_programming_fees = filter_input(INPUT_POST, 'cprogramming_fees');
        $inshop_car_programming_fees = filter_input(INPUT_POST, 'inshop_car_programming_fees');
		...
		/*****Lock Rekeying*******/
		$quantity_of_extra_house_keys = filter_input(INPUT_POST, 'quantity_of_extra_house_keys');
        $extra_house_key_price = filter_input(INPUT_POST, 'extra_house_key_price');
        $extra_house_key = filter_input(INPUT_POST, 'extra_house_key');
        $extra_per_house_key_price = filter_input(INPUT_POST, 'extra_per_house_key_price');
        ...
        $inshop_extra_per_house_key_price = filter_input(INPUT_POST, 'inshop_extra_per_house_key_price');
        $inshop_extra_per_cylinders_price = filter_input(INPUT_POST, 'inshop_extra_per_cylinders_price');

		/************************/
		
		/*******Car Lockout*******/
        $quantity_of_cars_unlocks = filter_input(INPUT_POST, 'quantity_of_cars_unlocks');
		/************************/
		
		/**********Home lockout***********/
		$quantity_of_door_locks_unlocks = filter_input(INPUT_POST, 'quantity_of_door_locks_unlocks');
		/*******************************/
		...
		$extra_deadbolt_cylinder_price = filter_input(INPUT_POST, 'extra_deadbolt_cylinder_price');
		$extra_deadbolt_cylinder = filter_input(INPUT_POST, 'extra_deadbolt_cylinder');
		$extra_per_deadbolt_cylinders_price = filter_input(INPUT_POST, 'extra_per_deadbolt_cylinders_price');
        ...
        $cart_item_data['booking_charge'] = $booking_charge;
        ...
        foreach ($my_location_address as $key => $value) {
			...
			if(!empty($total_miles_array[$key]['extra_amount'] )){
				$cart_item_data['total_miles_array'][$key] = $total_miles_array[$key];
			}
        ...
        if ($inshop_car_programming_fees > 0) {
            $cart_item_data['inshop_car_programming_fees'] = $inshop_car_programming_fees;
        }

        if ($inshop_car_key_price > 0) {
            $cart_item_data['inshop_car_key_price'] = $inshop_car_key_price;
        }
        ...
        if (!empty($quantity_of_locks_to_rekey)) {
            ...
            $cart_item_data['inshop_extra_per_cylinders_price'] = $inshop_extra_per_cylinders_price;
         } 
		 if (!empty($quantity_of_extra_house_keys)) {
            $cart_item_data['quantity_of_extra_house_keys'] = $quantity_of_extra_house_keys;
            $cart_item_data['extra_house_key_price'] = $extra_house_key_price;
            $cart_item_data['extra_house_key'] = $extra_house_key;
            $cart_item_data['extra_per_house_key_price'] = $extra_per_house_key_price;
            $cart_item_data['inshop_extra_per_house_key_price'] = $inshop_extra_per_house_key_price;
         }
        if (!empty($property_latitude) && !empty($property_longitude)) {
			...
			$cart_item_data['property_extra_price'] = ['total_miles_property'=>$total_miles_property,'extra_amount_property'=>$extra_amount_property,'maximum_miles'=>$maximum_miles,'default_miles'=>$default_miles,'extra_permile_price'=>$extra_permile_price];
        }
		...
		$cart_item_data['deal_type'] = $deal_type;
		/******Home Lockout*******/
        $cart_item_data['quantity_of_door_locks_unlocks'] = $quantity_of_door_locks_unlocks;
		/************/
		/******Car Lockout*******/
        $cart_item_data['quantity_of_cars_unlocks'] = $quantity_of_cars_unlocks;
		/************/
		
        if(!empty($extra_deadbolt_cylinder_price)){
			$cart_item_data['extra_deadbolt_cylinder_price']=$extra_deadbolt_cylinder_price;
			$cart_item_data['extra_deadbolt_cylinder']=$extra_deadbolt_cylinder;
			$cart_item_data['extra_per_deadbolt_cylinders_price']=$extra_per_deadbolt_cylinders_price;
		}
    ...
    function blsd_display_custom_text_cart($item_data, $cart_item) {
        ...
        if (!empty($cart_item['addons'])) {
            foreach ($cart_item['addons'] as $addon) {
                ...
                if (0 == $addon['price']) {
                    ...
                    $service_call_price .= wc_price(WC_Product_Addons_Helper::get_product_addon_price_for_display($cart_item['service_charge'], $cart_item['data'], true));
                } else {
                    $_product = new WC_Product($cart_item['product_id']);
                    $_product->set_price($price * ( $addon['price'] / 100 ));
                    $service_call_price .=  WC()->cart->get_product_price($_product);
                }
		...
		if($cart_item['deal_type'] == 'home_lockout' || $cart_item['deal_type'] == 'lock_rekeying' || $cart_item['deal_type'] == 'deadbolt'){
			...
			if (isset($cart_item['working_keys_locks'])) {
				$item_data[] = array(
					...
					'value' => wc_clean(ucfirst($cart_item['working_keys_locks'])),
					'display' => '',
				);
			}
			if($cart_item['deal_type'] == 'home_lockout'){
				if (isset($cart_item['quantity_of_door_locks_unlocks'])) {
				$item_data[] = array(
					'key' => __('Quantity of Door Locks to unlock', 'iconic'),
					'value' => wc_clean($cart_item['quantity_of_door_locks_unlocks']),
					'display' => '',
				);
			 }
			}
			else if($cart_item['deal_type'] == 'deadbolt'){
			...
			else if($cart_item['deal_type'] == 'lock_rekeying'){
				...
				if (isset($cart_item['quantity_of_extra_house_keys'])) {
					$item_data[] = array(
						'key' => __('Quantity of Duplicate House keys', 'iconic'),
						'value' => wc_clean($cart_item['quantity_of_extra_house_keys']),
						'display' => '',
					);
				 }
			}
		...
		else if($cart_item['deal_type'] == 'car_lockout' || $cart_item['deal_type'] == 'car_key_programming'){
        ...
        for ($i = 0; $i < $total_car; $i++) {
            ...
            if (isset($cart_item['serviceable_car'][$i]) && !empty($cart_item['serviceable_car'][$i])) {
            	...
            	if($cart_item['deal_type'] == 'car_lockout' || $cart_item['deal_type'] == 'car_key_programming'){
	                $item_data[] = array(
	                    'key' => __('Car Type' . $k, 'iconic'),
		...
		if (isset($cart_item['working_keys'])) {
            $item_data[] = array(
                ...
                'value' => wc_clean(ucfirst($cart_item['working_keys'])),
                'display' => '',
            );
        }
        ...
        if (isset($cart_item['when_start_car'])) {
            $item_data[] = array(
                ...
                'value' => wc_clean(ucfirst($cart_item['when_start_car'])),
                'display' => '',
            );
        }
        if (isset($cart_item['need_key_to_work'])) {
            $item_data[] = array(
                ...
                'value' => wc_clean(ucfirst($cart_item['need_key_to_work'])),
                'display' => '',
            );
        }
        if (isset($cart_item['car_currently_locked'])) {
            $item_data[] = array(
                ...
                'value' => wc_clean(ucfirst($cart_item['car_currently_locked'])),
                'display' => '',
            );
        }
			if($cart_item['deal_type'] == 'car_lockout'){
				if (isset($cart_item['quantity_of_cars_unlocks'])) {
				$item_data[] = array(
					'key' => __('Quantity of Car Locks to unlock', 'iconic'),
					// 'value' => wc_clean($cart_item['quantity_of_cars_unlocks']),
					'value' => '1',
					'display' => '',
				);
			 }
			}
			if($cart_item['deal_type'] == 'car_key_programming'){
				
				if(isset($cart_item['no_of_keys_want_to_made'])){    
					$item_data[] = array(
						'key' => __('Quantity of car keys you want cut and programmed', 'iconic'),
		...
		if($cart_item['deal_type'] == 'car_lockout'){
			if (isset($cart_item['will_owner_authorize_service'])) {
	            $item_data[] = array(
	                ...
	                'value' => wc_clean(ucfirst($cart_item['will_owner_authorize_service'])),
	                'display' => '',
	            );
        	}
		}else{
	        ...
	        if($service_call_price !=''){
				$item_data[] = array(
	                'key' => __('Service Call Price', 'iconic'),
	                'value' => wc_clean($service_call_price),
	                'display' => '',
	            );
			}
			if($cart_item['deal_type'] == 'car_key_programming'){
				if (isset($cart_item['inshop_car_key_price']) && $cart_item['inshop_car_key_price']>0) {
				}else{
					if(isset($cart_item['booking_charge'])){
						$item_data[] = array(
			                'key' => __('Labor to cut and program price', 'iconic'),
		...
		if (isset($cart_item['property_extra_price']) && !empty($cart_item['property_extra_price']['extra_amount_property'])) {
				$statement='('.$cart_item['property_extra_price']['total_miles_property'].'(Total Miles)'.'-'.$cart_item['property_extra_price']['default_miles'].'(Default Miles)) X '.$cart_item['property_extra_price']['extra_permile_price'].'(Per Mile price)';
					$price=get_woocommerce_currency_symbol().$cart_item['property_extra_price']['extra_amount_property'];
				$item_data[] = array(
					'key' => __('Extra Price', 'iconic'),
					'value' => wc_clean($statement.' = '.$price),
					'display' => '',
				);
		}

		if (isset($cart_item['extra_deadbolt_cylinder_price']) && !empty($cart_item['extra_deadbolt_cylinder_price'])) {
			$value_total=$cart_item['extra_deadbolt_cylinder']*$cart_item['extra_per_deadbolt_cylinders_price'];
			  $item_data[] = array(
				'key' => __($cart_item['extra_deadbolt_cylinder'].' (extra deadbolt)'.' X '.get_woocommerce_currency_symbol().$cart_item['extra_per_deadbolt_cylinders_price'].' (per deadbolt)', 'iconic'),
		...
		if (isset($cart_item['deadbolt_install_price']) && !empty($cart_item['deadbolt_install_price'])) {
		  ...
		  if($cart_item['inshop_extra_per_cylinders_price'] > 0 || $cart_item['inshop_extra_per_house_key_price'] > 0)
		  {
				  if (isset($cart_item['inshop_extra_per_cylinders_price']) ) {
					  ...
					  $value_total_per=$cart_item['inshop_extra_per_cylinders_price'] / $cart_item['quantity_of_locks_to_rekey'];
					  $item_data[] = array(
						'key' => __($cart_item['quantity_of_locks_to_rekey'].' (Total cylinder)'.' X '.get_woocommerce_currency_symbol().$value_total_per.' (In-Shop Standard Lock Rekeying Price) =', 'iconic'),
						'value' => wc_clean(get_woocommerce_currency_symbol().$value_total),
						'display' => '',
					);
				   
				  }
				  if (isset($cart_item['inshop_extra_per_house_key_price'])) {
					  $value_total=$cart_item['inshop_extra_per_house_key_price'];
					  $value_total_per=$cart_item['inshop_extra_per_house_key_price'] / $cart_item['quantity_of_extra_house_keys'];
					  $item_data[] = array(
						'key' => __($cart_item['quantity_of_extra_house_keys'].' (Total house keys)'.' X '.get_woocommerce_currency_symbol().$value_total_per.' (In-Shop Standard Key Duplication Price) =', 'iconic'),
						'value' => wc_clean(get_woocommerce_currency_symbol().$value_total),
						'display' => '',
					);
				  ...
				  if (isset($cart_item['extra_house_key']) && isset($cart_item['extra_per_house_key_price']) && !empty($cart_item['extra_house_key'])) {
					  $value_total=$cart_item['extra_house_key']*$cart_item['extra_per_house_key_price'];
					  $item_data[] = array(
						'key' => __($cart_item['extra_house_key'].' (extra house keys)'.' X '.get_woocommerce_currency_symbol().$cart_item['extra_per_house_key_price'].' (per house key) =', 'iconic'),
						'value' => wc_clean(get_woocommerce_currency_symbol().$value_total),
						'display' => '',
					);
		...
		for ($i = 0; $i < $total_car; $i++) {
            $k = '';
            if (isset($cart_item['total_miles_array'][$i]) && !empty($cart_item['total_miles_array'][$i])) {
				$statement='('.$cart_item['total_miles_array'][$i]['total_miles'].'(Total Miles)'.'-'.$cart_item['total_miles_array'][$i]['default_miles'].'(Default Miles)) X '.$cart_item['total_miles_array'][$i]['extra_permile_price'].'(Per Mile price)';
				$price=get_woocommerce_currency_symbol().$cart_item['total_miles_array'][$i]['extra_amount'];
				 $item_data[] = array(
                    'key' => __('Extra price' . $k, 'iconic'),
                    'value' => wc_clean($statement.''.$price),
                    'display' => '',
                );
			}
			...
			if (isset($cart_item['inshop_car_key_price']) && $cart_item['inshop_car_key_price']>0) {
				if (isset($cart_item['inshop_car_key_price']) && $cart_item['inshop_car_key_price']>0) {
					 $item_data[] = array(
						'key' => __('In-Shop Car Key Cutting Price', 'iconic'),
						'value' => wc_clean(get_woocommerce_currency_symbol().$cart_item['inshop_car_key_price']),
						'display' => '',
					); 
				}
				...
				if (isset($cart_item['inshop_car_key_price']) && $cart_item['inshop_car_key_price']>0) {
					 $item_data[] = array(
						'key' => __('In-Shop Car Key Programming Price', 'iconic'),
				...
				if(isset($cart_item['car_programming_fee']) && $cart_item['car_programming_fee'] >0  ){
					$item_data[] = array(
						...
						'key' => __('Additional Car Key Programming Price', 'iconic'),
						'value' => wc_clean(get_woocommerce_currency_symbol().$cart_item['car_programming_fee']),
						'display' => '',
					);
				}
				...
				if (isset($cart_item['car_key_type']) && isset($cart_item['car_key_price'])) {
					 $item_data[] = array(
						...
						'key' => __('Additional Car Key Cutting Price', 'iconic'),
    ...
    function blsd_add_custom_data_order_items($item, $cart_item_key, $values, $order) {
        /* if ( empty( $values['serviceable_car'] ) ) {
		  ...
		  if($values['deal_type'] == 'home_lockout' || $values['deal_type'] == 'lock_rekeying' || $values['deal_type'] == 'deadbolt'){
			...
			if($values['deal_type'] == 'home_lockout'){
				$item->add_meta_data(__('Quantity Of Door Locks to unlock', 'blsd'), $values['quantity_of_door_locks_unlocks']);
			}
			else if($values['deal_type'] == 'deadbolt'){
			...
			else if($values['deal_type'] == 'lock_rekeying'){
				$item->add_meta_data(__('Quantity of locks to rekey', 'blsd'), $values['quantity_of_locks_to_rekey']);
				$item->add_meta_data(__('Quantity of Duplicate House keys', 'blsd'), $values['quantity_of_extra_house_keys']);
			}
		  }
		  else if($values['deal_type'] == 'car_lockout' || $values['deal_type'] == 'car_key_programming'){
			 $total_car = count($values['serviceable_car']);
			 for ($i = 0; $i < $total_car; $i++) {
				$k = '';
				if($values['deal_type'] == 'car_key_programming' || $values['deal_type'] == 'car_lockout'){
					$item->add_meta_data(__('Car Type' . $k, 'blsd'), $values['serviceable_car'][$i]);
				}else{
				...
				/*$statement='('.$values['total_miles_array'][$i]['total_miles'].'(Total Miles)'.'-'.$values['total_miles_array'][$i]['default_miles'].'(Default Miles)) X '.$values['total_miles_array'][$i]['extra_permile_price'].'(Per mile price)';
				$price=get_woocommerce_currency_symbol().$values['total_miles_array'][$i]['extra_amount'];
				$item->add_meta_data(__('Extra price ' . $k, 'blsd'), $statement.' = '.$price); */
			 } 
			...
			if($values['deal_type'] == 'car_lockout'){
				 $item->add_meta_data(__('Quantity Of Car Locks to unlock', 'blsd'), $values['quantity_of_cars_unlocks']);
			}
			if($values['deal_type'] == 'car_key_programming'){
				if(!empty($values['no_of_keys_want_to_made'])){
					$item->add_meta_data(__('Quantity of car keys you want cut and programmed', 'blsd'), $values['no_of_keys_want_to_made']);
		  ...
		  $item->add_meta_data(__('Will the owner be able to authorize service?', 'blsd'), $values['will_owner_authorize_service']);
			...
			if(!empty($values['extra_deadbolt_cylinder_price'])){
				$value_total=$values['extra_deadbolt_cylinder']*$values['extra_per_deadbolt_cylinders_price'];
				$item->add_meta_data(__($values['extra_deadbolt_cylinder'].' (extra cylinder)'.' X '.get_woocommerce_currency_symbol().$values['extra_per_deadbolt_cylinders_price'], 'blsd'), get_woocommerce_currency_symbol().$value_total);
			}
			...
			{
				if (isset($values['inshop_extra_per_cylinders_price']) ) {
				  $value_total=$values['inshop_extra_per_cylinders_price'];
				  $value_total_per=$values['inshop_extra_per_cylinders_price'] / $values['quantity_of_locks_to_rekey'];
				  $item->add_meta_data(__($values['quantity_of_locks_to_rekey'].' (Total cylinder)'.' X '.get_woocommerce_currency_symbol().$value_total_per.' (In-Shop Standard Lock Rekeying Price)', 'blsd'), get_woocommerce_currency_symbol().$value_total);
			   
			    }
			    if (isset($values['inshop_extra_per_house_key_price'])) {
				  $value_total=$values['inshop_extra_per_house_key_price'];
				  $value_total_per=$values['inshop_extra_per_house_key_price'] / $values['quantity_of_extra_house_keys'];
				  $item->add_meta_data(__($values['quantity_of_extra_house_keys'].' (Total house keys)'.' X '.get_woocommerce_currency_symbol().$value_total_per.' (In-Shop Standard Key Duplication Price)', 'blsd'), get_woocommerce_currency_symbol().$value_total);
				...
				if(!empty($values['extra_cylinder'])){
					...
					$item->add_meta_data(__($values['extra_cylinder'].' (extra cylinder)'.' X '.get_woocommerce_currency_symbol().$values['extra_per_cylinders_price'], 'blsd'), get_woocommerce_currency_symbol().$value_total);
				}
				if(!empty($values['extra_house_key'])){
					$value_total=$values['extra_house_key']*$values['extra_per_house_key_price'];
					$item->add_meta_data(__($values['extra_house_key'].' (extra house keys)'.' X '.get_woocommerce_currency_symbol().$values['extra_per_house_key_price'].' (per house key)', 'blsd'), get_woocommerce_currency_symbol().$value_total);
				}
			}
			...
			if($values['deal_type'] == 'car_key_programming'){
				...
				if(!empty($values['inshop_car_key_price'])){
					if(!empty($values['inshop_car_programming_fees'])){
						$item->add_meta_data(__('In-Shop Car Programming Fee', 'blsd'), get_woocommerce_currency_symbol().$values['inshop_car_programming_fees']);
					}

					if(!empty($values['inshop_car_key_price'])){
						$item->add_meta_data(__('In-Shop Car Key Cutting Price', 'blsd'), get_woocommerce_currency_symbol().$values['inshop_car_key_price']);
					}
				}else{
					if(!empty($values['car_programming_fee'])){
						$item->add_meta_data(__('Additional Car Key Programming Price', 'blsd'), get_woocommerce_currency_symbol().$values['car_programming_fee']);
					}
					if(!empty($values['car_vats_fee'])){
						$item->add_meta_data(__('Car Vats Fee', 'blsd'), get_woocommerce_currency_symbol().$values['car_vats_fee']);
					}
					if(!empty($values['car_key_price'])){
						$item->add_meta_data(__('Additional Car Key Cutting Price', 'blsd'), get_woocommerce_currency_symbol().$values['car_key_price']);
					}
				}
			}else{
				if(!empty($values['car_programming_fee'])){
				...
				if(!empty($values['car_key_price'])){
					$item->add_meta_data(__('Your car key look like:'.$values['car_key_type'], 'blsd'), get_woocommerce_currency_symbol().$values['car_key_price']);
				}
			}
