...
class BuyLockSmithDealsCustomizationEmail {
    ...
    public static function blsd_email_on_submit_dispute($order, $customer_id, $title, $message_dispute) {

        $order_id = $order->get_order_number();
        $order_post = get_post($order_id);
        $parent_id = wp_get_post_parent_id($order_post);
        $product_table = '<table style="width:100%">
        <tr>
          <th>Order ID</th>
        ...
        if (isset($customer->user_email) && isset($product->ID)) {
            ...
            $message = 'Hello ' . $name . ',';
            $message .= '</br></br>';
            $message .= 'Dispute details are as below.';
            $message .= '</br></br>';
            $message .= 'Dispute Title: ' . $title;
            $message .= '</br></br>';
            $message .= 'Dispute Message: ' . $message_dispute;
            $message .= '</br></br>';
            ...
            $body='';
            $body .='<tr><td>';
            $body .='<span>Dispute Title:'.$title.'</span>';
            $body .='<span>Dispute Message:'.$message_dispute.'</span>';
            $body .='</br></br>';
            $body .='Product detail:';
            $body .=$product_table;
            ...
            $body .='</td></tr>';
             ...
             /*************************************/
                    ...
                    $mailer = WC()->mailer();
                    $template_html  = '/emails/custom_mail_template.php';
 ...
 <tr>
   ...
   <th>Product</th>
        ...
        if (isset($customer->user_email)) {
            ...
            $message .= 'Dispute details are as below.';
            $message .= '</br></br>';
            $message .='Status: '.BuyLockSmithDealsCustomizationAddon::blsd_get_status_name_by_id($status);
            $message .='</br></br>';
    ...
    public static function blsd_email_send_vendor_social_links_for_review($social_links, $customer_id) {
        ...
        if (isset($customer->user_email)) {
            ...
            foreach ($social_links as $key => $links) {
                if ($links != '') {
                    $message .= '<a href="' . $links . '">' . ucfirst($key) . '</a>';
                    $message .= '</br>';
                }
            }
