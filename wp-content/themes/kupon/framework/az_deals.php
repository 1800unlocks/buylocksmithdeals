...
$azexo_azd_templates = array(
    'best_rated_product' => esc_html__('Best rated product', 'AZEXO'),
    'latest_product' => esc_html__('Latest product', 'AZEXO'),
);
...
$azexo_azd_fields = array(
    'group_deal_purchases_left' => esc_html__('Deals: Group deal purchases left', 'AZEXO'),
    'deal_time_left' => esc_html__('Deals: Deal time left', 'AZEXO'),
    'price_offer' => esc_html__('Deals: Price offer', 'AZEXO'),
    'price_deal' => esc_html__('Deals: Price deal', 'AZEXO'),
    'price' => esc_html__('Deals: Price', 'AZEXO'),
    'price_trimmed' => esc_html__('Deals: Price (trimmed zeros)', 'AZEXO'),
    'product_rating' => esc_html__('Deals: Average product rating', 'AZEXO'),
    'add_to_cart' => esc_html__('Deals: Add to cart link', 'AZEXO'),
    'purchased_deals' => esc_html__('Deals: Single purchased deals', 'AZEXO'),
    'seller_info' => esc_html__('Deals: Single seller info', 'AZEXO'),
    'sold_by' => esc_html__('Deals: Sold by info', 'AZEXO'),
    'single_title' => esc_html__('Deals: Single product title', 'AZEXO'),
    'single_summary' => esc_html__('Deals: Single product summary', 'AZEXO'),
);
...
function azexo_azd_deal_time_left() {
    ...
    if (!empty($deal_expire)) {
        ...
        ?>
        <div class="time-left">
            <div class="call-to-action"><?php esc_html_e('Hurry up! Time left:', 'AZEXO'); ?></div>
            <div class="time" data-time="<?php print date('Y/m/d H:i:s', $deal_expire); ?>">
                <div class="days"><span class="count"><?php print $days; ?></span><span class="title"><?php print esc_html__('d.', 'AZEXO'); ?></span></div>
                <div class="hours"><span class="count"><?php print $hours; ?></span><span class="title"><?php print esc_html__('h.', 'AZEXO'); ?></span></div>
                <div class="minutes"><span class="count"><?php print $minutes; ?></span><span class="title"><?php print esc_html__('m.', 'AZEXO'); ?></span></div>
                <div class="seconds"><span class="count"><?php print $seconds; ?></span><span class="title"><?php print esc_html__('s.', 'AZEXO'); ?></span></div>
            </div>
        </div>
        <?php
    }
...
function azexo_azd_price_deal() {
    ...
    if (azexo_azd_is_on_sale($product)) {
        ...
        <div class="price-deal sale">
            <div class="regular-price">
                <?php echo apply_filters('woocommerce_get_price_html', apply_filters('woocommerce_price_html', $display_regular_price, $product), $product); ?>
                <span><?php esc_html_e('Value', 'AZEXO'); ?></span>
            </div>
            <div class="discount">
                <span class="amount"><?php print $discount; ?></span>
                <span><?php esc_html_e('Discount', 'AZEXO'); ?></span>
            </div>
            <div class="savings">
                <?php print $savings; ?>
                <span><?php esc_html_e('Savings', 'AZEXO'); ?></span>
            </div>
        </div>
        <?php
    } else {
        ...
        switch ($name) {
            ...
            case 'group_deal_purchases_left':
                if (function_exists('azd_is_group_off') && azd_is_group_off(get_the_ID())) {
                    ...
                    $output = '<div class="group-deal-sales-left"><span class="sales-left">' . esc_html($minimum_sales - $total_sales - $total_presales) . '</span> <span class="helper">' . esc_html__('purchases left to be deal on', 'AZEXO') . '</span></div>' . "\n";
                    return $output;
                }
                break;
            case 'purchased_deals':
                $deals = esc_html__('items', 'AZEXO');
                if (azexo_azd_is_on_sale($product)) {
                    $deals = esc_html__('deals', 'AZEXO');
                }
                $total_sales = (int) get_post_meta(get_the_ID(), 'total_sales', true);
                $total_presales = (int) get_post_meta(get_the_ID(), 'total_presales', true);
                $output = '<div class="purchased-deals-wrapper">';
                $output .= '<span class="deals">' . esc_html($total_sales + $total_presales) . ' ' . $deals . '</span>' . "\n";
                $output .= '<span class="purchased">' . esc_html__('Purchased', 'AZEXO') . '</span>';
                $output .= '</div>';
                return $output;
                break;
            case 'seller_info':
                $output = '<div class="title"><h3>' . esc_html__('Seller info', 'AZEXO') . '</h3></div>';
