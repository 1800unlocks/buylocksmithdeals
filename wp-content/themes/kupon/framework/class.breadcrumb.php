...
class AZEXO_Breadcrumb {
    ...
    private function add_crumbs_404() {
        $this->add_crumb(esc_html__('Error 404', 'AZEXO'));
    }
    ...
    private function add_crumbs_single($post_id = 0, $permalink = '') {
        ...
        if ('post' != get_post_type($current_post)) {
            $post_type = get_post_type_object(get_post_type($current_post));
            $this->add_crumb($post_type->labels->singular_name, get_post_type_archive_link(get_post_type($current_post)));
        } else {
            $cat = current(get_the_category($current_post));
            if ($cat) {
        ...
        $this->add_crumb(get_the_title($current_post), $permalink);
    }
    ...
    private function add_crumbs_tag() {
        ...
        $this->add_crumb(sprintf(esc_html__('Posts tagged &ldquo;%s&rdquo;', 'AZEXO'), single_tag_title('', false)), get_tag_link($queried_object->term_id));
    }
    ...
    private function add_crumbs_author() {
        ...
        $this->add_crumb(sprintf(esc_html__('Author: %s', 'AZEXO'), $userdata->display_name));
    }
    ...
    private function search_trail() {
        if (is_search()) {
            $this->add_crumb(sprintf(esc_html__('Search results for &ldquo;%s&rdquo;', 'AZEXO'), get_search_query()), remove_query_arg('paged'));
        }
    }
    ...
    private function paged_trail() {
        if (get_query_var('paged')) {
            $this->add_crumb(sprintf(esc_html__('Page %d', 'AZEXO'), get_query_var('paged')));
        }
    }
