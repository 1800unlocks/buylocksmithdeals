...
function azexo_admin_notices() {
    if (version_compare(phpversion(), '5.3.6', '<')) {
        ?>
        <div class="error">
            <p><?php esc_html_e('PHP version must be >= 5.3.6', 'AZEXO'); ?></p>
        </div>
        <?php
    }
}
...
function azexo_after_setup_theme() {
    load_theme_textdomain('AZEXO', get_template_directory() . '/languages');
...
function azexo_admin_scripts() {
    ...
    wp_localize_script('azexo-admin', 'azexo', array(
        ...
        'templates_configuration' => esc_html__('Templates configuration', 'AZEXO'),
        'fields_configuration' => esc_html__('Fields configuration', 'AZEXO'),
        'post_types_settings' => esc_html__('Post types settings', 'AZEXO'),
        'woocommerce_templates_configuration' => esc_html__('WooCommerce templates configuration', 'AZEXO'),
        'section_alert' => esc_html__('Please click "Ok" to confirm this info. Theme not provide CSS styles for all possible configurations in this section. Please use configurations only provided with demo content. Of course you can use it with complete freedom, but in this case it may be necessary to add CSS styles. You can save confirmation of this alert in "AZEXO Options > General settings".', 'AZEXO'),
...
function azexo_get_templates() {
    ...
    $azexo_templates = array_merge($azexo_templates, array(
        'post' => esc_html__('Post', 'AZEXO'), //default template
        'masonry_post' => esc_html__('Masonry post', 'AZEXO'), //fixed selector in JS
        'navigation_post' => esc_html__('Navigation post', 'AZEXO'), //fixed in full post navigation
        'related_post' => esc_html__('Related post', 'AZEXO'), //fixed in YARP template
    ));
...
function azexo_adminbarmenu($wp_admin_bar) {
    $args = array(
        'id' => 'edit-links',
        'title' => esc_html__('Edit links', 'AZEXO'),
        'href' => '#',
        'meta' => array(
            'class' => 'active',
        ),
    ...
    protected function comment($comment, $depth, $args) {
        ...
        <?php if ('div' != $args['style']) : ?>
            ...
            <div class="comment-data">
                <?php printf(wp_kses(__('<cite class="fn">%s</cite>', 'AZEXO'), array('cite' => array('class' => array()))), get_comment_author_link()); ?>
                <?php if ('0' == $comment->comment_approved) : ?>
                    <em class="comment-awaiting-moderation"><?php esc_html_e('Your comment is awaiting moderation.', 'AZEXO') ?></em>
                    <br />
                <?php endif; ?>
                <div class="comment-meta commentmetadata"><a href="<?php echo esc_url(get_comment_link($comment->comment_ID, $args)); ?>">
                        <?php
                        /* translators: 1: date, 2: time */
                        printf(esc_html__('%1$s at %2$s', 'AZEXO'), get_comment_date(), get_comment_time());
                        ?></a><?php edit_comment_link(esc_html__('(Edit)', 'AZEXO'), '&nbsp;&nbsp;', '');
                        ?>
                </div>
                <?php comment_text(get_comment_id(), array_merge($args, array('add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
                <div class="reply">
                    <?php comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'AZEXO'), 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
                </div>
...
if (function_exists('register_nav_menus')) {
    register_nav_menus(array(
        'primary' => esc_html__('Top primary menu', 'AZEXO'),
        'secondary' => esc_html__('Secondary menu', 'AZEXO'),
    ));
}
...
function azexo_widgets_init() {
    if (function_exists('register_sidebar')) {
        register_sidebar(array('name' => esc_html__('Right/Left sidebar', 'AZEXO'), 'id' => "sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Header sidebar', 'AZEXO'), 'id' => "header_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Middle sidebar', 'AZEXO'), 'id' => "middle_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Footer sidebar', 'AZEXO'), 'id' => "footer_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Dashboard sidebar', 'AZEXO'), 'id' => "dashboard_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Additional sidebar', 'AZEXO'), 'id' => "additional_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
    }
}
                    ...
                    case 'logo':
                        ...
                        <a class="site-title" href="<?php print esc_url(home_url('/')); ?>" rel="home">
                            ...
                            if (function_exists('has_custom_logo') && has_custom_logo()):
                                ...
                                ?>
                                <img src="<?php print esc_url($image_url); ?>" alt="<?php esc_attr_e('logo', 'AZEXO'); ?>">
                            <?php elseif (isset($options['logo']['url']) && !empty($options['logo']['url'])): ?>
                                <img src="<?php print esc_url($options['logo']['url']); ?>" alt="<?php esc_attr_e('logo', 'AZEXO'); ?>">
                            <?php else: ?>
...
function azexo_before_list($post_type) {
    ...
    if (isset($options[$post_type . '_custom_sorting_numeric_meta_keys']) && is_array($options[$post_type . '_custom_sorting_numeric_meta_keys'])) {
        foreach ($options[$post_type . '_custom_sorting_numeric_meta_keys'] as $meta_key) {
            if (!empty($meta_key)) {
                $orderby_options[esc_attr($meta_key) . '-desc'] = sprintf(esc_attr__('Sort by %s: high to low', 'AZEXO'), esc_attr($meta_key));
                $orderby_options[esc_attr($meta_key) . '-asc'] = sprintf(esc_attr__('Sort by %s: low to high', 'AZEXO'), esc_attr($meta_key));
            }
        }
    }
                ...
                switch ($part) {
                    case 'result_count':
                        ...
                        if ($total <= $per_page || -1 === $per_page) {
                            printf(_n('Showing the single result', 'Showing all %d results', $total, 'AZEXO'), $total);
                        } else {
                            printf(_nx('Showing the single result', 'Showing %1$d&ndash;%2$d of %3$d results', $total, '%1$d = first, %2$d = last, %3$d = total', 'AZEXO'), $first, $last, $total);
                        }
                        ?></p><?php
                        break;
                    case 'ordering':
...
function azexo_paging_nav() {
    ...
    $links = paginate_links(array(
        ...
        'prev_text' => '<i class="prev"></i>' . '<span>' . esc_html__('Previous', 'AZEXO') . '</span>',
        'next_text' => '<span>' . esc_html__('Next', 'AZEXO') . '</span>' . '<i class="next"></i>',
    ));
...
function azexo_get_the_category_list($separator = '', $parents = '', $post_id = false) {
    ...
    if (empty($categories)) {
        /** This filter is documented in wp-includes/category-template.php */
        return apply_filters('the_category', esc_html__('Uncategorized', 'AZEXO'), $separator, $parents);
    }
    ...
    if ($url) {
        ...
        <?php if (!is_single()): ?><a href="<?php esc_url(the_permalink()); ?>"><?php endif; ?>
            <?php if ($lazy): ?>
                ...
                <?php else: ?>
                    <img class="image lazy <?php print esc_attr($zoom); ?>" data-src="<?php print esc_url($url[0]); ?>" alt="<?php esc_attr_e('image', 'AZEXO'); ?>">
                <?php endif; ?>
            <?php else: ?>
                <?php if (preg_match('/\d+x\d+/', $thumbnail_size)): ?>
                ...
                <?php else: ?>
                    <img class="image <?php print esc_attr($zoom); ?>" src="<?php print esc_url($url[0]); ?>" alt="<?php esc_attr_e('image', 'AZEXO'); ?>">
                <?php endif; ?>
            <?php endif; ?>
...
function azexo_entry_field($name) {
    ...
    $image = (isset($options[$name . '_image']) && !empty($options[$name . '_image']['url'])) ? '<img src="' . esc_html($options[$name . '_image']['url']) . '" alt="' . esc_attr__('image', 'AZEXO') . '">' : '';
    ...
    switch ($name) {
        ...
        case 'post_sticky':
            if (is_sticky() && is_home() && !is_paged())
                return '<span class="featured-post">' . esc_html__('Sticky', 'AZEXO') . '</span>';
            break;
        ...
        case 'post_comments_count':
            if (comments_open()) {
                ...
                $comments = '<a href="' . esc_url(get_comments_link()) . '"><span class="count">' . $comment_count['total_comments'] . '</span><span class="label">' . ($comment_count['total_comments'] == 1 ? esc_html__('comment', 'AZEXO') : esc_html__('comments', 'AZEXO')) . '</span></a>';
                return '<span class="comments">' . $comments . '</span>';
            }
            break;
        case 'post_read_more':
            $more_link_text = sprintf(esc_html__('Read more', 'AZEXO'));
...
function azexo_comment_date($echo = true, $comment = null) {
    ...
    $options = get_option(AZEXO_FRAMEWORK);
    $date = sprintf('<span class="date">' . (isset($options['post_date_prefix']) ? esc_html($options['post_date_prefix']) : '') . '<a href="%1$s" title="%2$s" rel="bookmark"><time class="comment-date" datetime="%3$s">%4$s</time></a></span>', esc_url(get_comment_link($comment)), esc_attr(sprintf(esc_html__('Permalink to %s', 'AZEXO'), the_title_attribute(array('echo' => false, 'post' => $comment)))
        ...
        if (!empty($image_url)) {
            if ($lazy) {
                if (preg_match('/\d+x\d+/', $img_size)) {
                    ...
                    $output .= '<img class="image lazy" data-src="' . esc_url($image_url) . '" data-popup="' . esc_url($full_image_url) . '" alt="' . esc_attr__('image', 'AZEXO') . '">';
                }
            } else {
                if (preg_match('/\d+x\d+/', $img_size)) {
                    ...
                    $output .= '<img class="image" src="' . esc_url($image_url) . '" data-popup="' . esc_url($full_image_url) . '" alt="' . esc_attr__('image', 'AZEXO') . '">';
                }
            }
        }
    }
    ...
    if (!empty($date_to)) {
        ...
        <div class="time-left">
            ...
            <div class="time" data-time="<?php print date('Y/m/d H:i:s', $date_to); ?>">
                <div class="days"><span class="count"><?php print $days; ?></span><span class="title"><?php print (($days == 1) ? esc_html__('day', 'AZEXO') : esc_html__('days', 'AZEXO')); ?></span></div>
                <div class="hours"><span class="count"><?php print $hours; ?></span><span class="title"><?php print (($hours == 1) ? esc_html__('hour', 'AZEXO') : esc_html__('hrs', 'AZEXO')); ?></span></div>
                <div class="minutes"><span class="count"><?php print $minutes; ?></span><span class="title"><?php print esc_html__('min', 'AZEXO'); ?></span></div>
                <div class="seconds"><span class="count"><?php print $seconds; ?></span><span class="title"><?php print esc_html__('sec', 'AZEXO'); ?></span></div>
            </div>
        </div>
        <?php
    }
...
function azexo_infinite_scroll_js_options($options) {
    ...
    $options['loading']['msgText'] = '<em class="infinite-scroll-loading">' . esc_html__('Loading ...', 'AZEXO') . '</em>';
    $options['loading']['finishedMsg'] = '<em class="infinite-scroll-done">' . esc_html__('Done', 'AZEXO') . '</em>';
    return $options;
}
...
function azexo_get_dashboard_links() {
    ...
    $links = array(
        array(
            ...
            'title' => esc_html__('Log out', 'AZEXO'),
        )
    );
    $options = get_option(AZEXO_FRAMEWORK);
