...
$azexo_woo_templates = array(
    'single_product' => esc_html__('Single product', 'AZEXO'), //default template
    'shop_product' => esc_html__('Shop product', 'AZEXO'), //default template
    'detailed_shop_product' => esc_html__('Detailed shop product', 'AZEXO'), //fixed in shop modes
    'related_product' => esc_html__('Related product', 'AZEXO'), //fixed in related template
    'upsells_product' => esc_html__('Up-sells product', 'AZEXO'), //fixed in up-sells template
);
...
$azexo_woo_fields = array(
    'purchased' => esc_html__('Product purchased', 'AZEXO'),
    'discount' => esc_html__('Product discount', 'AZEXO'),
    'sale_time_left' => esc_html__('Product sale time left', 'AZEXO'),
    'availability' => esc_html__('Product availability', 'AZEXO'),
    'last_review_rating' => esc_html__('Product last review rating', 'AZEXO'),
    'loop_sale_flash' => esc_html__('Loop product sale flash', 'AZEXO'),
    'loop_rating' => esc_html__('Loop product average rating', 'AZEXO'),
    'loop_price' => esc_html__('Loop product price', 'AZEXO'),
    'loop_add_to_cart' => esc_html__('Loop product add to cart link', 'AZEXO'),
    'product_thumbnail' => esc_html__('Product thumbnail', 'AZEXO'),
    'product_gallery' => esc_html__('Product gallery', 'AZEXO'),
    'single_sale_flash' => esc_html__('Single product sale flash', 'AZEXO'),
    'single_add_to_cart' => esc_html__('Single product add to cart', 'AZEXO'),
    'single_rating' => esc_html__('Single product average rating', 'AZEXO'),
    'single_price' => esc_html__('Single product price', 'AZEXO'),
    'single_meta' => esc_html__('Single product meta', 'AZEXO'),
    'single_sharing' => esc_html__('Single product sharing', 'AZEXO'),
    'single_data_tabs' => esc_html__('Single data tabs', 'AZEXO'),
    'single_related_products' => esc_html__('Single related products', 'AZEXO'),
    'single_upsell_display' => esc_html__('Single upsell display', 'AZEXO'),
    'single_description' => esc_html__('Single description', 'AZEXO'),
    'single_reviews' => esc_html__('Single reviews', 'AZEXO'),
    'single_additional_information' => esc_html__('Single additional information', 'AZEXO'),
);

if (class_exists('WCV_Vendor_Shop')) {
    $azexo_fields['loop_sold_by'] = esc_html__('Loop product sold by', 'AZEXO');
}
if (class_exists('YITH_WCWL_Shortcode')) {
    $azexo_fields['add_to_wishlist'] = esc_html__('Add product to wishlist', 'AZEXO');
}
if (class_exists('YITH_WCQV_Frontend')) {
    $azexo_fields['quick_view'] = esc_html__('Product quick view', 'AZEXO');
}
...
function azexo_woo_settings_sections($sections) {
    ...
    $sections[] = array(
        'icon' => 'el-icon-cogs',
        'title' => esc_html__('WooCommerce templates configuration', 'AZEXO'),
        'fields' => array(
            array(
                'id' => 'shop_title',
                'type' => 'text',
                'title' => esc_html__('Shop page title', 'AZEXO'),
                'default' => 'Shop',
            ),
            array(
                'id' => 'product_label',
                'type' => 'text',
                'title' => esc_html__('Alter "Product" labels', 'AZEXO'),
                'default' => 'Product',
            ),
            array(
                'id' => 'products_label',
                'type' => 'text',
                'title' => esc_html__('Alter "Products" labels', 'AZEXO'),
                'default' => 'Products',
            ),
            array(
                'id' => 'review_form_placeholders',
                'type' => 'checkbox',
                'title' => esc_html__('Use placeholders in review form', 'AZEXO'),
                'default' => '0',
            ),
            array(
                'id' => 'custom_sorting',
                ...
                'options' => array(
                    ...
                    'popularity' => esc_html__('Sort by popularity', 'AZEXO'),
                    'rating' => esc_html__('Sort by average rating', 'AZEXO'),
                    'date' => esc_html__('Sort by newness', 'AZEXO'),
                    'price' => esc_html__('Sort by price: low to high', 'AZEXO'),
                    'price-desc' => esc_html__('Sort by price: high to low', 'AZEXO'),
                ),
            ...
            array(
                'id' => 'shop_modes',
                'type' => 'checkbox',
                'title' => esc_html__('Show shop modes', 'AZEXO'),
                'default' => '0',
            ),
            array(
                'id' => 'show_data_tabs',
                'type' => 'checkbox',
                'title' => esc_html__('Show data tabs', 'AZEXO'),
                'default' => '1',
            ),
            array(
                'id' => 'show_related_products',
                'type' => 'checkbox',
                'title' => esc_html__('Show related products', 'AZEXO'),
                'default' => '1',
            ),
            array(
                'id' => 'show_upsells_products',
                'type' => 'checkbox',
                'title' => esc_html__('Show upsells products', 'AZEXO'),
                'default' => '1',
            ),
            array(
                'id' => 'upsells_products_carousel_margin',
                'type' => 'text',
                'title' => esc_html__('Up-sells products carousel margin', 'AZEXO'),
                'default' => '0',
            ),
            array(
                'id' => 'related_products_carousel_margin',
                'type' => 'text',
                'title' => esc_html__('Related products carousel margin', 'AZEXO'),
                'default' => '0',
            ),
            array(
                'id' => 'show_ratings_summary',
                'type' => 'checkbox',
                'title' => esc_html__('Show ratings summary', 'AZEXO'),
                'default' => '0',
            ),
            array(
                'id' => 'review_marks',
                'type' => 'multi_text',
                'title' => esc_html__('Review marks', 'AZEXO'),
            ),
            array(
                'id' => 'review_likes',
                'type' => 'checkbox',
                'title' => esc_html__('Review likes', 'AZEXO'),
                'default' => '0',
            ),
        )
    );
...
function azexo_woo_tgmpa_register() {
    ...
    $plugins = array(
        array(
            'name' => esc_html__('WooCommerce', 'AZEXO'),
            'slug' => 'woocommerce',
            'required' => true,
        ),
    );
    if (class_exists('WooCommerce')) {
        $plugins[] = array(
            'name' => esc_html__('WC Vendors', 'AZEXO'),
            'slug' => 'wc-vendors',
        );
    }
    tgmpa($plugins, array());
...
function azexo_woo_taxonomy_args_product_cat($args) {
    if (!is_admin()) {
        ...
        if (isset($options['product_label']) && !empty($options['product_label'])) {
            $args['label'] = $options['product_label'] . ' ' . esc_html__('Categories', 'AZEXO');
            $args['labels']['name'] = $options['product_label'] . ' ' . esc_html__('Categories', 'AZEXO');
            $args['labels']['singular_name'] = $options['product_label'] . ' ' . esc_html__('Category', 'AZEXO');
        }
    }
    return $args;
}
...
function azexo_woo_taxonomy_args_product_tag($args) {
    if (!is_admin()) {
        $options = get_option(AZEXO_FRAMEWORK);
        if (isset($options['product_label']) && !empty($options['product_label'])) {
            $args['label'] = $options['product_label'] . ' ' . esc_html__('Tags', 'AZEXO');
            $args['labels']['name'] = $options['product_label'] . ' ' . esc_html__('Tags', 'AZEXO');
            $args['labels']['singular_name'] = $options['product_label'] . ' ' . esc_html__('Tag', 'AZEXO');
        }
    }
    return $args;
}
        ...
        if (!is_user_logged_in()) {
            foreach ($sorted_menu_items as $i => &$menu_item) {
                if ($menu_item->object_id == $woocommerce_myaccount_page_id && $menu_item->object == 'page') {
                    $menu_item->title = esc_html__('Login/Register', 'AZEXO');
                }
            }
        }
    }
...
function azexo_woo_menu_start_el($item, $args) {
    ...
    if (is_array($item->classes)) {
        if (in_array('hot', $item->classes)) {
            ...
            $item->additions .= '<span class="hot">' . esc_html__('Hot', 'AZEXO') . '</span>';
        }
        if (in_array('new', $item->classes)) {
            $item->classes = array_diff($item->classes, array('new'));
            $item->additions .= '<span class="new">' . esc_html__('New', 'AZEXO') . '</span>';
        }
    }
    return $item;
}
...
azexo_add_element(array(
    ...
    'category' => esc_html__('WooCommerce', 'AZEXO'),
...
azexo_add_element(array(
    'name' => esc_html__('Cart widget', 'AZEXO'),
    'base' => 'woocommerce_cart_widget',
    'icon' => 'icon-wpb-woocommerce',
    'category' => esc_html__('WooCommerce', 'AZEXO'),
    'description' => esc_html__('Displays the cart contents', 'AZEXO'),
    'show_settings_on_create' => false,
    'params' => array(
        array(
            'type' => 'textfield',
            ...
            'value' => esc_html__('Cart', 'AZEXO'),
        ),
        array(
            'type' => 'checkbox',
            'heading' => esc_html__('Hide if cart is empty', 'AZEXO'),
            'param_name' => 'hide_if_empty',
            'value' => array(
                esc_html__('Yes, please', 'AZEXO') => 1,
            ),
        ),
        array(
            'type' => 'textfield',
...
function azexo_woo_catalog_orderby($sortby) {
    ...
    if (isset($options['custom_sorting_numeric_meta_keys']) && is_array($options['custom_sorting_numeric_meta_keys'])) {
        foreach ($options['custom_sorting_numeric_meta_keys'] as $meta_key) {
            if (!empty($meta_key)) {
                $sortby[esc_attr($meta_key) . '-desc'] = sprintf(esc_attr__('Sort by %s: high to low', 'AZEXO'), esc_attr($meta_key));
                $sortby[esc_attr($meta_key) . '-asc'] = sprintf(esc_attr__('Sort by %s: low to high', 'AZEXO'), esc_attr($meta_key));
            }
        }
    }
    return $sortby;
...
add_action('woocommerce_update_product', 'azexo_woo_update_product', 10, 1);

function azexo_woo_update_product($product_id) {
    azexo_woo_variable_product_sync($product_id);
}
