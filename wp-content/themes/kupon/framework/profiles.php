...
function azexo_azlp_templates($azexo_templates) {
    $azexo_templates['single_profile'] = esc_html__('Single profile', 'AZEXO');
    $azexo_templates['list_profile'] = esc_html__('List profile', 'AZEXO');
...
function azexo_azlp_fields($azexo_fields) {
    return array_merge($azexo_fields, array(
        'profile_thumbnail' => esc_html__('Profile: Thumbnail', 'AZEXO'),
        'profile_rating' => esc_html__('Profile: Average rating', 'AZEXO'),
        'profile_review_count' => esc_html__('Profile: Review count', 'AZEXO'),
        'profile_last_review_rating' => esc_html__('Profile: last review rating', 'AZEXO'),
        'profile_link' => esc_html__('Profile: Link to author profile', 'AZEXO'),
        'profile_products_count' => esc_html__('Profile: Products count', 'AZEXO'),
    ));
}
...
function azexo_azlp_settings_sections($sections) {
    ...
    $sections[] = array(
        'icon' => 'el-icon-cogs',
        'title' => esc_html__('Profiles templates configuration', 'AZEXO'),
        'fields' => array(
            array(
                'id' => 'profile_label',
                'type' => 'text',
                'title' => esc_html__('Alter "Profile" labels', 'AZEXO'),
                'default' => 'Profile',
            ),
            array(
                'id' => 'profiles_label',
                'type' => 'text',
                'title' => esc_html__('Alter "Profiles" labels', 'AZEXO'),
                'default' => 'Profiles',
            ),
            array(
                'id' => 'redirect_product_to_profile',
                'type' => 'checkbox',
                'title' => esc_html__('Redirect product to profile', 'AZEXO'),
                'default' => '0',
            ),
        )
    );
...
function azexo_azlp_entry_field($output, $name) {
    ...
    switch ($name) {
        ...
        case 'profile_products_count':
            ...
            if (!empty($posts)) {
                $profile = reset($posts);
                $products = '<a href="' . esc_url(get_permalink($profile)) . '"><span class="count">' . esc_html($count) . '</span><span class="label">' . esc_html__('products', 'AZEXO') . '</span></a>';
        ...
        case 'profile_link':
            ...
            if (!empty($posts)) {
                ...
                $label = (isset($options['profile_link_prefix']) && !empty($options['profile_link_prefix'])) ? esc_html($options['profile_link_prefix']) : esc_html__('Profile', 'AZEXO');
		...
		", $post->ID));
            ...
            if ($rating_count > 0) {
                ob_start();
                ?>
                <div class="star-rating" title="<?php printf(esc_html__('Rated %s out of 5', 'AZEXO'), $average); ?>">
                    <span style="width:<?php echo ( ( $average / 5 ) * 100 ); ?>%">
                        <strong class="rating"><?php echo esc_html($average); ?></strong> <?php printf(esc_html__('out of %s5%s', 'AZEXO'), '<span>', '</span>'); ?>
                        <?php printf(_n('based on %s user rating', 'based on %s user ratings', $rating_count, 'AZEXO'), '<span class="rating">' . $rating_count . '</span>'); ?>
                    </span>
                </div>
                <?php
                $output = ob_get_clean();
        ...
        case 'profile_review_count':
            ...
            if (comments_open()) {
                ob_start();
                ?>
                <a href="#reviews" class="profile-review-link" rel="nofollow">(<?php printf(_n('%s customer review', '%s customer reviews', $count, 'AZEXO'), '<span class="count">' . $count . '</span>'); ?>)</a>
                <?php
                $output = ob_get_clean();
            }
            break;
        case 'profile_last_review_rating':
            ...
            if ($comment) {
                ...
                if ($rating) {
                    $rating_html = '<div class="star-rating" title="' . sprintf(esc_html__('Rated %s out of 5', 'AZEXO'), $rating) . '">';
                    $rating_html .= '<span style="width:' . ( ( $rating / 5 ) * 100 ) . '%"><strong class="rating">' . $rating . '</strong> ' . esc_html__('out of 5', 'AZEXO') . '</span>';
                    $rating_html .= '</div>';
                    $output = $rating_html;
                }
            }
            ...
            if ($profile) {
                ...
                $links = array_merge(array(
                    array(
                        ...
                        'title' => esc_html__('My Profile', 'AZEXO'),
                    ),
                    array(
                        'id' => $edit_profile,
                        'url' => esc_url(add_query_arg(array('azl' => 'edit', 'id' => $profile->ID))),
                        'title' => esc_html__('Edit Profile', 'AZEXO'),
                    ),), $links);
            }
        }
    }
            ...
            foreach ($vendors as $vendor) {
                ...
                if (isset($vendor['minimum_order_amount'])) {
                    if (floatval($total) < floatval($vendor['minimum_order_amount'])) {
                        wc_add_notice(sprintf(esc_html__('%s: minimum order amount %s, current order total is %s.', 'AZEXO'), $vendor['profile']->post_title, wc_price($vendor['minimum_order_amount']), wc_price($total)), 'error');
                    }
                }
            }
            wc_print_notices();
