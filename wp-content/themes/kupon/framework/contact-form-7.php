...
function azexo_before_send_mail($contact_form) {
    ...
    if ($submission) {
        ...
        if (isset($posted_data['recipient_email'])) {
            $properties = $contact_form->get_properties();
            $properties['mail']['recipient'] = $posted_data['recipient_email'];
            $contact_form->set_properties($properties);
        }
        if (isset($posted_data['recipient_phone'])) {
            $properties = $contact_form->get_properties();
            $properties['mail']['recipient'] = $posted_data['recipient_phone'] . AZEXO_EMAIL_SMS_GATEWAY;
            $contact_form->set_properties($properties);
        }
    }
}
...
function azexo_special_mail_tags($output, $name, $html) {
    ...
    if (strpos($name, 'post-meta-') == 0) {
        if (isset($posted_data['_post_id'])) {
            return get_post_meta($posted_data['_post_id'], str_replace('post-meta-', '', $name), true);
        } else {
            return '';
        }
    }
