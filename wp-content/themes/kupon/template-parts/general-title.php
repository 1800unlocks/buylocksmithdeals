...
<?php if (is_404()) : ?>
    <div class="page-header">
        <h1 class="page-title"><?php esc_html_e('Not Found', 'AZEXO'); ?></h1>
    </div>
<?php elseif (is_category()): ?>
    <div class="archive-header">
        <h1 class="archive-title"><?php print single_cat_title('', false); ?></h1>
        <div class="archive-subtitle"><?php print esc_html__('Category archives', 'AZEXO'); ?></div>
...
<?php elseif (is_tag()): ?>
    <div class="archive-header">
        ...
        <div class="archive-subtitle"><?php print esc_html__('Tag archives', 'AZEXO'); ?></div>
    ...
    <div class="archive-header">
        <h1 class="archive-title"><?php
            if (is_day()) :
                print get_the_date();
            elseif (is_month()) :
                print get_the_date(_x('F Y', 'monthly archives date format', 'AZEXO'));
            elseif (is_year()) :
                print get_the_date(_x('Y', 'yearly archives date format', 'AZEXO'));
            endif;
            ?></h1>
        <div class="archive-subtitle"><?php
            if (is_day()) :
                esc_html_e('Daily Archives', 'AZEXO');
            elseif (is_month()) :
                esc_html_e('Monthly Archives', 'AZEXO');
            elseif (is_year()) :
                esc_html_e('Yearly Archives', 'AZEXO');
            endif;
            ?></div>
    </div><!-- .archive-header -->
...
<?php elseif (is_search()): ?>
    <div class="page-header">
        <h1 class="page-title"><?php print esc_html__('Search Results for', 'AZEXO'); ?></h1>
