...
<div class="price-drop-alert">
    <?php if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'price-drop-alert') : ?>
        ...
        ?>
        <p class="info"><?php esc_html_e('Price drop alert created', 'AZEXO'); ?></p>
    <?php else: ?>
        <?php
        ...
        <?php if ($expected_price) : ?>
            <p class="info"><label><?php esc_html_e('Expected price drop alert:', 'AZEXO'); ?></label> <span class="expected-price"><?php print wc_price($expected_price); ?></span><a href="<?php print esc_url(add_query_arg('action', 'price-drop-alert-remove')); ?>" class="remove"><?php esc_attr_e('Remove', 'AZEXO'); ?></a></p>
        <?php else: ?>
            <form method="post" action="<?php print esc_url(get_permalink()); ?>">
                <input type="hidden" name="action" value="price-drop-alert" />
                <p class="expected-price"><input type="number" name="expected-price" value="" placeholder="<?php esc_attr_e('Expected price', 'AZEXO'); ?>" /></p>
                <?php if (!is_user_logged_in()) : ?>
                    <p class="email"><input type="email" name="email" required="required" value="" placeholder="<?php esc_attr_e('Your email', 'AZEXO'); ?>" /></p>
                <?php endif; ?>
                <p class="submit"><input type="submit" value="<?php esc_html_e('Get price-drop alert', 'AZEXO'); ?>" /></p>
            </form>
        <?php endif; ?>
    <?php endif; ?>
</div>