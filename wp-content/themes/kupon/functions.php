...
function azexo_scripts() {
    ...
    if (in_array('administrator', (array) $user->roles)) {
        ...
        $frontend_variables['edit_button'] = esc_html__('Edit', 'AZEXO');
        ...
        if (function_exists('get_redux_instance')) {
            ...
            $frontend_variables['edit_links']['configure_templates'] = array(
                ...
                'text' => esc_html__('Configure template', 'AZEXO'),
                'target' => '_blank',
            );
...
function azexo_styles() {
    ...
    if (class_exists('WPLessPlugin')) {        
        ...
        $less->dispatch();
        $skin_style = false;
        if (file_exists(get_template_directory() . '/less/' . azexo_get_skin() . '/skin.less')) {
    ...
    if (isset($options['google_font_families']) && is_array($options['google_font_families'])) {
        ...
        foreach ($options['google_font_families'] as $font_family) {
            if ('off' !== esc_html_x('on', $font_family . ' font: on or off', 'AZEXO')) {
...
add_action('init', 'azexo_fix_google_font_families', 13); // after load default skin options

function azexo_fix_google_font_families() {
    $options = get_option(AZEXO_FRAMEWORK);
    if (isset($options['google_font_families']) && is_array($options['google_font_families'])) {
        $font_families = array();
        foreach ($options['google_font_families'] as $font_family) {
