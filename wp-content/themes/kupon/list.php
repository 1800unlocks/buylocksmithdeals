...
<div class="<?php print ((isset($options['content_fullwidth']) && $options['content_fullwidth']) ? '' : 'container'); ?> <?php print (is_active_sidebar('sidebar') && ($show_sidebar != 'hidden') ? 'active-sidebar ' . esc_attr($show_sidebar) : ''); ?> <?php print (in_array('list', $additional_sidebar) ? 'additional-sidebar' : ''); ?>">
    ...
    <div id="primary" class="content-area">
        ...
        ?>
        <div id="content" class="site-content <?php print esc_attr(str_replace('_', '-', $template_name)); ?> <?php print ((isset($options['infinite_scroll']) && $options['infinite_scroll']) ? 'infinite-scroll' : '') ?>" role="main">
            <?php
            if (isset($options['author_bio']) && $options['author_bio'] && get_the_author_meta('description')) {
                if (is_author()) {
