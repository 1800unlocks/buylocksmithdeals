...
if ($query->have_posts()) {
    ...
    if (!empty($filter)) {
        ...
        if (is_array($filter_all_terms) && !empty($filter_all_terms)) {
            ...
            print '<div class="filter-term active">' . esc_html__('All', 'AZEXO') . '</div>';
    ...
    if (empty($group)) {
        ...
        foreach ($groups as $group_name => $posts) {
            ...
            print '<input id="group-toggle-' . hash($group_name) . '" type="checkbox" style="position: absolute; clip: rect(0, 0, 0, 0);">';
            print '<div id="list-group-' . hash($group_name) . '" class="group-header"><h4>' . esc_html($group_name) . '</h4><label for="group-toggle-' . hash($group_name) . '"></label></div>';
